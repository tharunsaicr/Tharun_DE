...
df_emp_dump=spark.sql("select * from kgsonedatadb.trusted_hist_headcount_employee_dump where File_Date = '20230228'and Entity = 'KGS'")
...
df_com_yec=spark.sql("select * from kgsonedatadb.trusted_hist_compensation_yec where File_Date = '20230201'").withColumnRenamed("EMP_CATEGORY","Emp_Category").withColumnRenamed("Location","Location1").withColumnRenamed("FULL_NAME","Full_Name1").withColumnRenamed("Post_Graduation___Highest_Qualification","Post_Graduation_Highest_Qualification");
...
# df_join_emp.select("Work_experience_prior_to_KPMG__Years_").distinct().display()
...
# df_add_col.select("Work_experience_prior_to_KPMG__Years_").distinct().display()

# COMMAND ----------

# df_add_col = df_add_col.withColumn("PriorExp_Check",col("Work_experience_prior_to_KPMG__Years_"))
...
# DfCurrent.select("Work_experience_prior_to_KPMG__Years_").distinct().display()

# display(DfCurrent.select("Work_experience_prior_to_KPMG__Years_","PriorExp_Check"))
...
df_leave = df_leave.withColumn("Total_Unapid_Leaves_Taken",sum("Leave_Count").over(windowSpec))

df_leave = df_leave.select("Employee_Number","Name","Total_Unapid_Leaves_Taken").distinct()
...
DfCurrent=DfCurrent.join(df_leave,DfCurrent.Emp_ID==df_leave.Employee_Number,'left').select(DfCurrent['*'],df_leave.Total_Unapid_Leaves_Taken)
...
# display(DfCurrent.filter(DfCurrent.BU.isNull()))
