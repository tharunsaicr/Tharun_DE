...
currentDf=currentDf.withColumnRenamed("Amount","Net_Amount").withColumn('MMM',upper(substring(col("Month"),0,3)))
currentDf=currentDf.withColumn("Calendar_Year",when(currentDf['MMM'].isin(month_list),currentDf['Financial_Year']-1).otherwise((currentDf['Financial_Year']))).withColumn("Net_Amount",col("Net_Amount").cast('double')).withColumn('Month_Key',concat(col("Calendar_Year"),from_unixtime(unix_timestamp(col("MMM"),'MMM'),'MM')))
