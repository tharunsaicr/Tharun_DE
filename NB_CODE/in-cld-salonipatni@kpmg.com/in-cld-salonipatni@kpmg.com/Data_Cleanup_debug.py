...
# Added on 5/19/2023 - due to badrecords failure in LND
# spark.sql("set spark.databricks.checkLatestSchemaOnRead=FALSE")
...
currentDf = spark.sql("select * from kgsonedatadb.raw_hist_"+processName + "_"+ tableName + " where File_Date = '20200519'")
...
for columnName in currentDf.columns:
    ...
    if (columnName in dateList):
        ...
        # display(currentDf.select("Date_First_Hired", "last_TK_APPROVED_LWD", "last_Termination_Date", "Termination_Date","DateConversion"))
        print(columnName)
        # display(currentDf)
...
# currentDf.filter(col('DateConversion') == "Date invalid").display()
...
goodDf = currentDf.filter((~(upper(currentDf.IntConversion) == 'INT TYPE CAST ISSUE')) & (~(upper(currentDf.FloatConversion) == 'FLOAT TYPE CAST ISSUE')) & (~(upper(currentDf.DoubleConversion) == 'DOUBLE TYPE CAST ISSUE')) & (~(upper(currentDf.LongConversion) == 'LONG TYPE CAST ISSUE'))& (~(upper(currentDf.TimeStampConversion) == 'TIMESTAMP TYPE CAST ISSUE')) & (~(upper(currentDf.DateConversion) == 'DATE INVALID')) & ((upper(currentDf.Is_Primary_Duplicate_Record) != 'PK DUPLICATE RECORD') ))


badDf = currentDf.filter((upper(currentDf.Is_Duplicate_Record) == 'DUPLICATE RECORD') | (upper(currentDf.Is_Primary_Key_Null) == 'PK IS NULL') | (upper(currentDf.Is_Primary_Duplicate_Record) == 'PK DUPLICATE RECORD') | (upper(currentDf.IntConversion) == 'INT TYPE CAST ISSUE') | (upper(currentDf.FloatConversion) == 'FLOAT TYPE CAST ISSUE') | (upper(currentDf.DoubleConversion) == 'DOUBLE TYPE CAST ISSUE') | (upper(currentDf.LongConversion) == 'LONG TYPE CAST ISSUE')| (upper(currentDf.TimeStampConversion) == 'TIMESTAMP TYPE CAST ISSUE') | (upper(currentDf.DateConversion) == 'DATE INVALID') | (upper(currentDf.Is_Column_Null) == 'SOURCE NOT NULL') | (upper(currentDf.Is_Primary_Duplicate_Record) == 'PK DUPLICATE RECORD'))
...
goodDf.select('LWD').distinct().display()

# COMMAND ----------

# Commenting on 5/19/2023 for LND and uncommented after load
if (processName.lower() != 'lnd') & (tableName.lower() != 'glms_kva_details'):
    print("inside")
    from pyspark.sql.types import *
    from pyspark.sql.functions import col, unix_timestamp, to_date
    ...
    for columnName in currentDf.columns:
        ...
        if (columnName in dateList):
            ...
            .when(to_date(currentDf[columnName], 'dd MMMM, yyyy').isNotNull(),to_date(currentDf[columnName], 'dd MMMM,yyyy'))\
            .when(unix_timestamp(currentDf[columnName], 'dd-MM-yy').isNotNull(),from_unixtime(unix_timestamp(currentDf[columnName], 'dd-MM-yy'),'yyyy-MM-dd'))\
            .when(unix_timestamp(currentDf[columnName], 'dd-MMM-yy').isNotNull(),from_unixtime(unix_timestamp(currentDf[columnName], 'dd-MMM-yy'),'yyyy-MM-dd'))\
            .when(unix_timestamp(currentDf[columnName], 'dd MMM yy').isNotNull(),from_unixtime(unix_timestamp(currentDf[columnName], 'dd MMM yy'),'yyyy-MM-dd'))\
            .when(to_date(currentDf[columnName], 'yyyy/MM/dd').isNotNull(),to_date(currentDf[columnName], 'yyyy/MM/dd'))\
            .otherwise(currentDf[columnName]))


            columnNew = columnName+'_New'


            currentDf=currentDf.withColumn(columnNew,\
            when(unix_timestamp(currentDf[columnName], 'dd-MM-yy').isNotNull(),from_unixtime(unix_timestamp(currentDf[columnName], 'dd-MM-yy'),'dd-MM-yyyy'))\
            .when(unix_timestamp(currentDf[columnName], 'dd-MMM-yy').isNotNull(),from_unixtime(unix_timestamp(currentDf[columnName], 'dd-MMM-yy'),'dd-MM-yyyy'))\
            .when(unix_timestamp(currentDf[columnName], 'dd MMM yy').isNotNull(),from_unixtime(unix_timestamp(currentDf[columnName], 'dd MMM yy'),'dd-MM-yyyy'))\
            .otherwise(currentDf[columnName]))
            
            display(currentDf.select(columnName,columnNew).distinct())
    ...
    # Added during LND history load 5/19/2023
        ...
        if (columnName in doubleList):
            # currentDf= currentDf.withColumn(columnName,currentDf[columnName].cast(FloatType()))
            currentDf= currentDf.withColumn(columnName,when((((currentDf[columnName].isNotNull())  & (trim(currentDf[columnName]) != "") ) & currentDf[columnName].cast(DoubleType()).isNotNull()),currentDf[columnName].cast(DoubleType()))\
            .otherwise(currentDf[columnName]))
...
#             currentDf=currentDf.withColumn(columnName,\
#             when(to_date(currentDf[columnName], 'dd-MM-yy').isNotNull(),from_unixtime(unix_timestamp(currentDf[columnName], 'dd-MM-yy'),'yyyy-MM-dd'))\
#             .when(to_date(currentDf[columnName], 'dd-MMM-yy').isNotNull(),from_unixtime(unix_timestamp(currentDf[columnName], 'dd-MMM-yy'),'yyyy-MM-dd'))\
#             .when(to_date(currentDf[columnName], 'dd MMM yy').isNotNull(),from_unixtime(unix_timestamp(currentDf[columnName], 'dd MMM yy'),'yyyy-MM-dd'))\
#             .otherwise(currentDf[columnName]))
...
currentDf.select('LWD').distinct().display()

# COMMAND ----------

# DBTITLE 1,Column Typecast
# Uncommented on 5/19/2023 to load lnd and commented above one and commented after load
#Uncomment when reloading the whole data
# 
# 
if (processName.lower() == 'lnd') & (tableName.lower() == 'glms_kva_details'):
    print("inside")
    ...
    for columnName in currentDf.columns:
        ...
        if (columnName in dateList):
            currentDf = currentDf.withColumn(columnName,currentDf[columnName].cast(DateType()))
        ...
        if (columnName in timestampList):
            currentDf = currentDf.withColumn(columnName,currentDf[columnName].cast(TimestampType()))

        if (columnName in doubleList):
            currentDf = currentDf.withColumn(columnName,currentDf[columnName].cast(DoubleType()))
...
# Commented on 5/19/2023 due to LND error and uncommented post load
