...
if tableName=='analytics':
    spark.sql('''Update kgsonedatadb.trusted_hist_gdc_onboarding_master
              SET LATERAL_CAMPUS= CASE 
                                        when (LATERAL_CAMPUS="Professional") then "Lateral"
                                        when LATERAL_CAMPUS="Lateral" then "Lateral"
                                        WHEN ((DESIGNATION="Associate 1") and ((SERVICE_LINE="Canada Audit") or (SERVICE_LINE="US Core Audit"))) then "PG Campus" WHEN ((DESIGNATION="Associate 2") and ((SERVICE_LINE="Canada Audit") or (SERVICE_LINE="US Core Audit"))) then "CA Campus" 
                                        else "Campus"
                                    end''')
...
if tableName  in ("absconding","active_hc","attrition","attrition_pipeline","long_leave","fts","secondee","transfers","contractors"):
    ...
    case when (a.PSID=regexp_replace(b.PS_ID,'[^0-9]','') or a.PSID=c.PS_ID or a.PSID=d.PSID or a.PSID=e.US_ID or a.PSID=z.PS_ID) and (i.PS_ID is null) and (f.PS_ID is null) and (g.PS_ID is null) then 'Active'
    ...
    from kgsonedatadb.trusted_hist_gdc_onboarding_master a left join kgsonedatadb.trusted_gdc_active_hc b on a.PSID=regexp_replace(b.PS_ID,'[^0-9]','')
    ...
    left join kgsonedatadb.trusted_gdc_contractors z on a.PSID=regexp_replace(z.PS_ID,'[^0-9]','')
...
if tableName=='laptop_delivery':
    spark.sql('''MERGE INTO kgsonedatadb.trusted_hist_gdc_onboarding_master a 
               USING  kgsonedatadb.config_gdc_training_days b
               ON a.SERVICE_LINE=b.Service_Line and a.LOCATION='Hyderabad' and a.LAPTOP_REACHED_ON is not null
               WHEN MATCHED  THEN  
                UPDATE SET a.TARGET_TRAINING_COMPLETION_DATE=a.LAPTOP_REACHED_ON+b.Total_days''')
...
if tableName=='risk_check':
    spark.sql('''MERGE INTO kgsonedatadb.trusted_hist_gdc_onboarding_master a 
               ...
               ON regexp_replace(a.PSID,'[^0-9]','') =regexp_replace(b.PSID,'[^0-9]','') 
               WHEN MATCHED  THEN  
...
if tableName=="ready_for_production":
    spark.sql('''create or replace temporary view rmt_vw as (select * from (select *,row_number() over(partition by PSID,EMPID order by HANDOVER asc) as rn from kgsonedatadb.trusted_gdc_ready_for_production) a where rn=1)''')
...
# MAGIC %sql
# MAGIC SET TIME ZONE 'Asia/Kolkata';
...
spark.sql(''' 
          Update kgsonedatadb.trusted_hist_gdc_onboarding_master
            ...
            SET TIME_TAKEN_TO_RECEIVE_LND_CLEARANCE_DATE_FROM_LAPTOP_RECEIVED_DATE= CASE 
                                    ...
                                    end,Dated_On=from_utc_timestamp(CURRENT_TIMESTAMP(),'Asia/Kolkata')''')
