...
# currentDf = currentDf.select('LOCATION','DEPARTMENT','EMPLOYEE_ID','EMPLOYEE_NAME','STATUS2','FROM_DATE','STATUS','BU','Original_DEPARTMENT','HRMS_DEPARTMENT')
...
currentDf = currentDf.withColumn('DEPARTMENT', \
    when((currentDf.DEPARTMENT.isNull()) | (currentDf.DEPARTMENT == '-'), col('Original_DEPARTMENT'))\
    .otherwise(col('DEPARTMENT')))
...
display(currentDf.filter(col('BU').isNull()))
