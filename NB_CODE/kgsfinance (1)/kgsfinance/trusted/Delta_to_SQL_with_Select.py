...
if (reportName =='fr') and (processName =='lnd'):
    ...
    if table == 'trusted_curr_fr_lnd_outlook_cost':
        year= spark.sql('select distinct Financial_Year from kgsfinancedb.trusted_curr_fr_lnd_outlook_cost')
        fy=year.select('Financial_year').rdd.flatMap(lambda x: x).collect()
        fy=fy[0]
        sql_command = f"SELECT COUNT(*) FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = '{hist_table}'"
        cursor.execute(sql_command)
    ...
    if table == 'trusted_curr_fr_lnd_outlook_cost_full_year':
        year= spark.sql('select distinct Financial_Year from kgsfinancedb.trusted_curr_fr_lnd_outlook_cost_full_year')
        fy=year.select('Financial_year').rdd.flatMap(lambda x: x).collect()
        fy=fy[0]
        sql_command = f"SELECT COUNT(*) FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = '{hist_table}'"
        cursor.execute(sql_command)
...
if (reportName =='fr') and (processName =='admin'):
    ...
    if table == 'trusted_curr_fr_admin_seats_actual':
        ...
        result = cursor.fetchone()[0]
        if result == 1:
            query=(""" DELETE FROM dbo.{} where Financial_Year in {} """).format(hist_table,fy)
            print(query)
            conn.execute(query)
            conn.commit()
            print('delete executed')
...
if (reportName =='fr') and (processName =='recruitment'):
    ...
    if table == 'trusted_curr_fr_recruitment_team_cost_outlook':
        year= spark.sql('select distinct Financial_year from kgsfinancedb.trusted_curr_fr_recruitment_team_cost_outlook')
        fy=year.select('Financial_year').rdd.flatMap(lambda x: x).collect()
        fy=fy[0]
        print(fy)
        sql_command = f"SELECT COUNT(*) FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = '{hist_table}'"
        ...
        sampleDF=spark.sql('select Entity, Operating_Unit, Cost_Center, Cost_Center_ID, Cost_Center_Name, Compensation_Type, Geo, Account_mapping, BU, BU_mapping_as_per_console, Service_line, Financial_Year, Dated_On, Month, Value, Calendar_Year, Month_Key, FY, Period from kgsfinancedb.'+table)
    ...
    if table == 'trusted_curr_fr_recruitment_hiring_outlook':
        year= spark.sql('select distinct Financial_year from kgsfinancedb.trusted_curr_fr_recruitment_hiring_outlook')
        fy=year.select('Financial_year').rdd.flatMap(lambda x: x).collect()
        fy=fy[0]
        print(fy)
        sql_command = f"SELECT COUNT(*) FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = '{hist_table}'"
        ...
        sampleDF=spark.sql('select Employee_Number, Full_Name, Function, Employee_Subfunction, Employee_Subfunction_1, Organization_Name, Cost_centre, BU_as_per_HR, Operating_Unit, User_Type, Client_Geography, Location, Sub_Location, Position, Job, People_Group, Employee_Category, Date_First_Hired, End_Date_for_FTS, Gender, Company, Supervisor_Name, Performance_Manager, email, Payroll_Contractor, Count, Geo, Geo_Total, Geo_2, Designation, Designation_for_Cost_line, Designation_for_hiring_Mix, Month_Hired, Month_number_hired, Period, Source, Source_2, Final_BU_Mapping, Entity, Service_line, Financial_Year, Dated_On, Month, Calendar_Year, Month_Key from kgsfinancedb.'+table)
    ...
    if table == 'trusted_curr_fr_recruitment_cost_outlook':
        year= spark.sql('select distinct Financial_year from kgsfinancedb.trusted_curr_fr_recruitment_cost_outlook')
        fy=year.select('Financial_year').rdd.flatMap(lambda x: x).collect()
        fy=fy[0]
        print(fy)
        sql_command = f"SELECT COUNT(*) FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = '{hist_table}'"
        ...
        sampleDF=spark.sql('select Entity, Ledger_Name, Operating_Unit, Business_Category, Client_Geo, GDC_Future, GDC_Future_1, Inter_Operating_Unit, User_Type, Source, Function, Sub_Function, Service_Line, Profit_Centre_Cost_Centre, Profit_Cost_Centre_Code, Function_Code, Debit_Amt, Credit_Amt, Net_Amount, Location, MI_Grouping, Project_ID, Account_Code, Account_Name, Account_Flex_Field, Category, Account_Grouping, Project_Org, Project_Name, Project_Description, Vendor_Name, Customer_Name, Period, Invoice_No_ER_No_Adjustment_No, Description_Full, GL_Description, Batch_Name, Journal_Desc, Journal_Name, Justification_from_ER, PO_Number, Payment_Doc_No, Payment_Date, Created_By, CREATED_BY__DESC, Creation_Date, Creation_Time, GL_Date, DD_UTR_Number, Receipt_Creator_Name, Receipt_Number, Vendor_PAN_No, Customer_PAN_No, Vendor_Address, Customer_Address, Vendor_Service_Tax_RegNo, Customer_Service_Tax_RegNo, Requisition_Number, Requisition_Description, MI_Mapping, MI_Mapping_1, Additional_Comments, Source_1, Spoc, Remarks, Date, Type, CC_Geo, Affiliate_Operating_Unit, Affliate_MI_Grouping, Inter_Entity, Common_Cost_Entity, Common_Cost_Location, 1_Check___MI_Mapping, 2_Opearting_Unit_check__per_MI_Mapping, Bonus_In_Percent, Markup_In_Percent, Bonus, Gratuity___LE, Total_Incl_Bonus___Gratuity, Total_Incl_Markup, Remarks_1, Revenue, GEO_Final, BU_Wise, MI_Mapping_2, BU, GEO, GEO_revised, Geo_mapping_for_total, revised_BU, Category_1, Category_2, BU_mapping_as_per_console, PY_CY, YTD, Service_Line_Final, Comment, Financial_Year, Dated_On, Month, Calendar_Year, Month_Key from kgsfinancedb.'+table)
...
if (reportName =='fr') and (processName =='it'):
    ...
    if table == 'trusted_curr_fr_it_hc_forecast':
        ...
        .option("password", password) \
        .save()
    if table == 'trusted_curr_fr_it_hc_outlook':
        year= spark.sql('select distinct Financial_year from kgsfinancedb.trusted_curr_fr_it_hc_outlook')
        fy=year.select('Financial_year').rdd.flatMap(lambda x: x).collect()
        fy=fy[0]
        print(fy)
        sql_command = f"SELECT COUNT(*) FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = '{hist_table}'"
    ...
    if table == 'trusted_curr_fr_it_cost_outlook':
        year= spark.sql('select distinct Financial_year from kgsfinancedb.trusted_curr_fr_it_cost_outlook')
        fy=year.select('Financial_year').rdd.flatMap(lambda x: x).collect()
        fy=fy[0]
        print(fy)
        sql_command = f"SELECT COUNT(*) FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = '{hist_table}'"
    ...
    if table == 'trusted_curr_fr_it_cfit_hc_outlook':
        year= spark.sql('select distinct Financial_year from kgsfinancedb.trusted_curr_fr_it_cfit_hc_outlook')
        fy=year.select('Financial_year').rdd.flatMap(lambda x: x).collect()
        fy=fy[0]
        print(fy)
        sql_command = f"SELECT COUNT(*) FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = '{hist_table}'"
        ...
        .option("password", password) \
        .save()
        print("cfit_hc_outlook")
 
 
    if table == 'trusted_curr_fr_it_cfit_cost_outlook':
        year= spark.sql('select distinct Financial_year from kgsfinancedb.trusted_curr_fr_it_cfit_cost_outlook')
        fy=year.select('Financial_year').rdd.flatMap(lambda x: x).collect()
        fy=fy[0]
        print(fy)
        sql_command = f"SELECT COUNT(*) FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = '{hist_table}'"
