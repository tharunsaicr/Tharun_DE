...
df_currentED = spark.sql("select * from kgsonedatadb.raw_hist_headcount_employee_dump where File_Date = '20230606'")
...
df_currentTKStatus = spark.sql("select EMPLOYEENUMBER,REQUESTSTATUS,APPROVED_LWD from kgsonedatadb.raw_curr_headcount_talent_konnect_resignation_status_report")
...
df_lastcutoff_TKStatus = spark.sql("select EMPLOYEENUMBER,REQUESTSTATUS,APPROVED_LWD from (select rank() over(partition by EMPLOYEENUMBER,file_date order by dated_on desc) as rank, * from kgsonedatadb.trusted_hist_headcount_talent_konnect_resignation_status_report where file_date = (select max(file_date) from kgsonedatadb.trusted_hist_headcount_talent_konnect_resignation_status_report where to_date(File_Date,'yyyyMMdd') < to_date("+"'"+current_cutoff_date+"'"+"))) tk_hist where rank = 1")
...
df_currentED.filter(col('Employee_Number') == '103983').display()
