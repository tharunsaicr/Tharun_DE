...
if processName == "jml":
    ...
    if table == "trusted_jml_uk_joiner":
        sampleDF=spark.sql('select ID,Priority,Employee_ID,Candidate_India_ID,Start_Date,First_Name,Last_Name,Grade,Employee_Group,Work_Schedule,Contract_Activity,Position,Function,Cost_centre,Cost_Centre_Description,Capability,Service_Line_Code,Service_Line_Description,Location,Employee_Status,Previous_staff_number,Gender,Date_of_Birth,PML_Staff_Number,PML_Name,IT_Sub_function,Type_of_Employee,Profile_Type,Additional_Applications,Status,GPID,UK_Staff_ID,Login_ID,Password,VDI_Image,Email,Client,VDI_Link,UK_MI_Resource,Comments,Created,Created_By,HR_Date,JML_Date,People_Center_Date,ITS_Date,MI_Date,Dated_On,File_Date,ED_Employee_Number,ED_Full_Name,ED_Function,ED_Employee_Subfunction,ED_Employee_Subfunction_1,ED_Cost_centre,ED_Client_Geography,ED_Position,ED_GPID,left(File_Date,6) as Month_Key from kgsonedatadb.'+table)
    ...
    if table == "trusted_jml_uk_leaver":
        sampleDF=spark.sql('select ID,Priority,UK_Staff_ID as Title,First_Name,Last_Name,Leave_Date,UK_Email_Address,Reason_For_Leaving,`Action_Required?`,UK_People_Center,Status,Comments,HR_Date,JML_Date,People_Center_Date,Created_By,Dated_On,File_Date,ED_Employee_Number,ED_Full_Name,ED_Function,ED_Employee_Subfunction,ED_Employee_Subfunction_1,ED_Cost_centre,ED_Client_Geography,ED_Position,ED_UK_Employee_Number,left(File_Date,6) as Month_Key from kgsonedatadb.'+table)
    ...
    if table == "trusted_jml_uk_mover":
        sampleDF=spark.sql('select ID,Process_Type,Priority,Effective_Date,UK_Staff_ID,First_Name,Last_Name,Request_Type,New_Function,New_Cost_Centre,Absence_End_Date,Absence_Type,New_Capability,Service_Line_Code,Service_Line_Description,ITS_Sub_Function_to_be_revoked,ITS_Sub_Function_to_be_provided,DMS_environment_worksite_has_been_removed,Profile_Type,New_Grade,Employee_Group,Work_Schedule,Contract_Activity_Type,Office,Job_Title,PML_Staff_Number,Secondment_End_Date,Additional_Applications,Status,PML_Staff_Name,UK_People_Center,UK_Resourcing_MI,Global_Mobility_team,HR_Date,JML_Date,People_Center_Date,MI_Date,ITS_Date,GM_Date,Created_By,Dated_On,File_Date,ED_Employee_Number,ED_Full_Name,ED_Function,ED_Employee_Subfunction,ED_Employee_Subfunction_1,ED_Cost_centre,ED_Client_Geography,ED_Position,ED_UK_Employee_Number,left(File_Date,6) as Month_Key from kgsonedatadb.'+table)
...
if processName == "TA_post_hire":
    ...
    if table == "trusted_TA_post_hire_CWK":
        sampleDF=spark.sql('select Req__Identifier,Candidate_Identifier,Candidate_Name,Designation,Cost_Centre,BU,Geo,Recruiter_Name,Email,Hiring_Source,Project_Code,Start_Date,Joining_Location,Sub_Location,Location_Category,Exception_Approval_To_On_Board,ZONE__For_Delhi__NCR_,Phone_Number,Current_Address_With_landmark_where_Laptop_Can_be_Delivered,Candidate_Category__ROD_or_RPO_,Remarks,KGS_Email_ID,Status,Updated_DOJ,BGV_Reference_No_,BGV_Status,Report_Colour,Mandatory_Checks_Completed,All_Checks_Completed,Case_Remarks,Onboarding_Approval,Approval_Date,Final_Joining_Status,Dated_On,left(File_Date,6) as Month_Key from kgsonedatadb.'+table)
        sampleDF.write \
        .format("jdbc") \
        .mode("overwrite") \
        .option("truncate", "true") \
...
if processName == "employee_engagement":
    ...
    if table == "trusted_employee_engagement_encore_output":
        sampleDF=spark.sql('select Emp_No,Final_Name,Designation,BU,Nominee_s_Unit,Category,Reward_Status,Quarter,Year,Email_ID,Level,Gender,Gratuity_Date,Company_Name,Budget_From,Nominator_s_Employee_ID,Reward_Date,Nomination_Date,File_Date,Dated_On,left(File_Date,6) as Month_Key from kgsonedatadb.'+table)
    ...
    if table == "trusted_employee_engagement_rock":
        sampleDF=spark.sql('select Employee_Number,Full_Name,Function,Sub_Function,Cost_centre,BU,Location,Position,Employee_Category,Gratuity_Date,Date_First_Hired,Email_Address,Company_Name,desired_LWD,Approved_LWD,RM_Employee_Name,Rock_Tenure,CAST(Amount as decimal),Tenure_Month,Tenure_Year,Tenure_as_on,File_Date,Dated_On,left(File_Date,6) as Month_Key from kgsonedatadb.'+table)
...
if processName == "headcount":
    ...
    if table == "trusted_headcount_employee_details":
        ...
        sampleDF = spark.sql('''select * from kgsonedatadb.'''+hist_table)
        ...
        if BusinessCategoryAvailable.upper() == 'YES':
            sampleDF=spark.sql('''select Employee_Number,Full_Name,Function,Employee_Subfunction,Employee_Subfunction_1,Organization_Name,Cost_centre,Operating_Unit,User_Type,Client_Geography,Location,Sub_Location,Position,Job_Name,People_Group_Name,Employee_Category,Date_First_Hired,End_Date,Gender,Company_Name,Supervisor_Name,Performance_Manager,Email_Address,BU,Remarks,'' as Current_Base_Location_of_the_Candidate,'' as Office_Location_in_the_KGS_Offer_Letter,'' as WFA_Option__Permanent_12_Months_,Dated_On,File_Date,Business_Category,left(File_Date,8) as Month_Key from kgsonedatadb.'''+hist_table)
        else:
            sampleDF=spark.sql('''select Employee_Number,Full_Name,Function,Employee_Subfunction,Employee_Subfunction_1,Organization_Name,Cost_centre,Operating_Unit,User_Type,Client_Geography,Location,Sub_Location,Position,Job_Name,People_Group_Name,Employee_Category,Date_First_Hired,End_Date,Gender,Company_Name,Supervisor_Name,Performance_Manager,Email_Address,BU,Remarks,'' as Current_Base_Location_of_the_Candidate,'' as Office_Location_in_the_KGS_Offer_Letter,'' as WFA_Option__Permanent_12_Months_,Dated_On,File_Date,'' as Business_Category,left(File_Date,8) as Month_Key from kgsonedatadb.'''+hist_table)
    ...
    if table == "trusted_headcount_leave_report":
        ...
        # sampleDF=spark.sql('''select *,concat(year(leave_start_date),date_format(leave_start_date,"MM")) as Month_Key from kgsonedatadb.'''+hist_table+''' where file_Date = "20230522"''')
    ...
    if table == "trusted_headcount_attrition_data":
        ...
        sampleDF.write.format("jdbc").mode("overwrite").option("url",jdbcUrl).option("dbtable",hist_table).option("user", username).option("password", password).save()
...
if processName == "headcount_monthly":
    if table == "trusted_headcount_monthly_contingent_worker":
        ...
        Subcontract_Agency,Official_Email_ID,Assignment_Category,Staff_Type,BU,File_Date,File_Month,File_Year,Dated_On,Position,People_Group,Gender,Company,left(File_Date,6) as Month_Key from kgsonedatadb.'''+hist_table)
    ...
    if table == "trusted_headcount_monthly_contingent_worker_resigned":
        ...
        Official_Email_ID,Assignment_Category,BU,File_Date,File_Month,File_Year,Dated_On,left(File_Date,6) as Month_Key from kgsonedatadb.'''+hist_table)
    ...
    if table == "trusted_headcount_monthly_employee_details":
        ...
        Gender,Company_Name,Supervisor_Name,Performance_Manager,Email_Address,BU,'' as Remarks,Current_Base_Location_of_the_Candidate,Office_Location_in_the_KGS_Offer_Letter,WFA_Option__Permanent_12_Months_,File_Date,File_Month,File_Year,Dated_On,left(File_Date,6) as Month_Key from kgsonedatadb.'''+hist_table)
    ...
    if table == "trusted_headcount_monthly_loaned_staff_from_ki":
        ...
        CASE when (End_Date== ' ' or End_Date== '-' ) then NULL else End_Date END as End_Date,
        Position_Name,Email_Id,File_Date,File_Month,File_Year,Dated_On,left(File_Date,6) as Month_Key from kgsonedatadb.'''+hist_table)
    ...
    if table == "trusted_headcount_monthly_loaned_staff_resigned":
        ...
        File_Date,Dated_On,left(File_Date,6) as Month_Key,File_Month,File_Year from kgsonedatadb.'''+hist_table)
    ...
    if table == "trusted_headcount_monthly_resigned_and_left":
        ...
        Gender,Employee_Category,People_Group_Name,Company_Name,Email_Address,BU,File_Date,File_Month,File_Year,Dated_On,left(File_Date,6) as Month_Key from kgsonedatadb.'''+hist_table)
    ...
    if table == "trusted_headcount_monthly_sabbatical":
        ...
        ,File_Date,File_Month,File_Year,Dated_On,left(File_Date,6) as Month_Key from kgsonedatadb.'''+hist_table)
    ...
    if table == "trusted_headcount_monthly_secondee_outward":
        sampleDF=spark.sql('''select Employee_Number,Full_Name,Function,Employee_Subfunction,Employee_Subfunction_1,Organization_Name,Cost_centre,Operating_Unit,User_Type,Client_Geography,Location,Sub_Location,Position,Job_Name,People_Group_Name,Employee_Category,
        CASE when (Date_First_Hired == ' ' or Date_First_Hired == '-') then NULL else Date_First_Hired END as Date_First_Hired,
        End_Date,Gender,Company_Name,Supervisor_Name,Performance_Manager,Email_Address,BU,File_Date,File_Month,File_Year,Dated_On,left(File_Date,6) as Month_Key from kgsonedatadb.'''+hist_table)
    ...
    if table == "trusted_headcount_monthly_academic_trainee":
        sampleDF=spark.sql('''select Candidate_Id,Full_Name,Function,Employee_Subfunction,Employee_Subfunction_1,Organization_Name,Cost_Centre,Operating_Unit,Client_Geography,User_Type,Location,Sub_Location,Job,Position,People_Group,
        CASE when (Date_of_Joining== ' ' or Date_of_Joining== '-') then NULL else Date_of_Joining END as Date_of_Joining,
        Gender,Company,Subcontract_Agency,Official_Email_ID,Assignment_Category,Staff_Type,BU,File_Date,File_Month,File_Year,Dated_On,left(File_Date,6) as Month_Key from kgsonedatadb.'''+hist_table)
    ...
    if table == "trusted_headcount_monthly_maternity_cases":
        ...
        CASE when (Start_Date == ' ' or Start_Date == '-') then NULL else Start_Date END as Start_Date,
        CASE when (End_Date == ' ' or End_Date == '-') then NULL else End_Date END as End_Date,
        Status,BU,File_Date,File_Month,File_Year,Dated_On,left(File_Date,6) as Month_Key from kgsonedatadb.'''+hist_table)
...
if processName == "compensation_non_sensitive":
    if table == "trusted_compensation_non_sensitive_qualification_dump":
        sampleDF=spark.sql('''select EMP_ID,FULL_NAME,Post_Graduation___Highest_Qualification,GRAD,OTHERS,Dated_On,File_Date,left(File_Date,6) as Month_Key from kgsonedatadb.'''+hist_table)
        # sampleDF=spark.sql('''select *,left(File_Date,6) as Month_Key from kgsonedatadb.'''+hist_table)
        sampleDF.write \
        .format("jdbc") \
        .mode("overwrite") \
        .option("url",jdbcUrl) \
...
#         sampleDF=spark.sql('select EMP_ID,FULL_NAME,FUNCTION_1,SUB_FUNCTION,SUB_FUNCTION1,ORGANIZATION,LOCATION,DESIGNATION,JOB_CODE,Pitched_at__Current_Positioning_,cast(Date_since_at_current_designation as date) as Date_since_at_current_designation,Years_at_current_designation__YY_MM_,cast(DOJ as date) as DOJ,Years_in_the_Firm__YY_MM_,Work_experience_prior_to_KPMG__Years_,TOTAL_WORK_EXP_INCL_KPMG,cast(STARTDATE as date) as STARTDATE,CTC,Previous_Year_CTC,Previous_Year_Performance_Bonus___Prorated_,Previous_Year_Sales_Bonus,Previous_Year_Special_Bonus,Total_Bonus_of_Previous_Year,Bonus_Accrued,RATING_OCT_2021_TO_SEP_2022_,RATING_OCT_2020_TO_SEP_2021_,RATING_OCT_2019_TO_SEP_2020_,RATING_OCT_2018_TO_SEP_2019_,"" as RATING_OCT_2017_TO_SEP_2018_,Post_Graduation___Highest_Qualification,GRAD,OTHERS,NUMBER_OF_LEAVES,Employee_Status__Employee___Serving_Notice_,EMAIL,COMPANY,EMP_CATEGORY,Dated_On,File_Date,RATING_OCT_2022_TO_SEP_2023_,left(File_Date,6) as Month_Key from kgsonedatadb.'+table)
...
#         sampleDF=spark.sql('select Cost_centre,Position,Client_Geography,Aggregated_HC,Annual_Aggregated_CTC,Dated_On,File_Date,left(File_Date,6) as Month_Key from kgsonedatadb.'+table)
...
if processName == "it":
    if table == "trusted_it_optional_business_application_installation":
        ...
        Dated_On,File_Date,left(File_Date,6) as Month_Key from kgsonedatadb.'''+hist_table)
    ...
    if table == "trusted_it_talent_connect_daily_transfer_report":
        ...
        left(File_Date,8) as Month_Key from kgsonedatadb.'''+hist_table)
...
if processName == "admin":
    if table == "trusted_admin_qlikview_report":
        sampleDF=spark.sql('''select ENTITY_NAME,OPERATING_UNIT,VENDOR_NUMBER,VENDOR_NAME,MSME_STATUS,PO_AMOUNT,PO_NUMBER,INVOICE_NUMBER,PO_DATE,PAYMENT_METHOD,DESCRIPTION,STATUS,PROJECT_NUMBER,PROJECT_DESCRIPTION,CHEQUE_DATE,CHEQUE_AMOUNT,CHEQUE_NO___DOC_NO,BUYER_NAME,REQUESTER,REQUESTER_LOCATION,DD_NO__UTR,BATCH_NAME,Dated_On,FILE_DATE,left(File_Date,8) as Month_Key,INVOICE_DATE from kgsonedatadb.'''+hist_table)
...
Table_list = ['trusted_hist_impact_access_card_data',
'trusted_hist_impact_accident_within_office_premises',
'trusted_hist_impact_admin_data',
'trusted_hist_impact_air_travel_data',
'trusted_hist_impact_car_lease_data',
'trusted_hist_impact_csr_data',
'trusted_hist_impact_electricity_and_dg_data',
'trusted_hist_impact_employee_commute_data',
'trusted_hist_impact_fatality_within_office_premises',
'trusted_hist_impact_fuel_reimbursement_leased_cars',
'trusted_hist_impact_gps_data',
'trusted_hist_impact_gps_wellbeing',
'trusted_hist_impact_idne_gender_target_vs_actuals',
'trusted_hist_impact_integrity_training_data',
'trusted_hist_impact_it_purchased_goods_consumption_data',
'trusted_hist_impact_kgs_rec_purchases',
'trusted_hist_impact_office_safety_compliance',
'trusted_hist_impact_official_travel_personal_car',
'trusted_hist_impact_one_to_one_status_data',
'trusted_hist_impact_purchased_goods_and_services',
'trusted_hist_impact_sick_wellbeing_data',
'trusted_hist_impact_waste_management_data',
'trusted_hist_impact_wlb_attrition_data',
'trusted_hist_impact_fuel_reimbursement_leased_cars']
...
for table in Table_list:
    ...
    if table == "trusted_hist_impact_kgs_rec_purchases":
        sampleDF=spark.sql('''select FY,QUANTITY_OF_REC_PURCHASES_KWH,Dated_On,FILE_DATE,left(File_Date,6) as Month_Key from kgsonedatadb.'''+hist_table)
    ...
    if table == "trusted_hist_impact_fuel_reimbursement_leased_cars":
    ...
    if table == "trusted_hist_impact_sick_wellbeing_data":
        sampleDF=spark.sql('''select YEAR,MONTH,KGS,CONSULTING_AVG_UTILIZATION,KRC_AVG_UTILIZATION,TAX_AVG_UTILIZATION,GDC_AVG_UTILIZATION,DAS_AVG_UTILIZATION,CORPORATE_FUNCTIONS_AVG_UTILIZATION,RAS_AVG_UTILIZATION,DN_AVG_UTILIZATION,MS_AVG_UTILIZATION,CAPABILITY_HUBS_AVG_UTILIZATION,ADVISORY_AVG_UTILIZATION,
        Dated_On,FILE_DATE,left(File_Date,6) as Month_Key from kgsonedatadb.'''+hist_table)
    ...
    if table == "trusted_hist_impact_one_to_one_status_data":
    ...
    if table == "trusted_hist_impact_gps_wellbeing":
    ...
    if table == "trusted_hist_impact_wlb_attrition_data":
        sampleDF=spark.sql('''select YEAR,MONTH,KGS_PERCENTAGE,KGS_EXCLUDING_GDC_PERCENTAGE,CONSULTING_PERCENTAGE,KRC_PERCENTAGE,
                     TAX_PERCENTAGE,GDC_PERCENTAGE,DAS_PERCENTAGE,CORPORATE_FUNCTIONS_PERCENTAGE,RAS_PERCENTAGE,DN_PERCENTAGE,MS_PERCENTAGE,
                     CAPABILITY_HUBS_PERCENTAGE,ADVISORY_PERCENTAGE,
        Dated_On,FILE_DATE,left(File_Date,6) as Month_Key from kgsonedatadb.'''+hist_table)
    ...
    if table == "trusted_hist_impact_fatality_within_office_premises":
    ...
    if table == "trusted_hist_impact_office_safety_compliance":
    ...
    if table == "trusted_hist_impact_accident_within_office_premises":
        sampleDF=spark.sql('''select YEAR,MONTH,CONSULTING,KRC,TAX,GDC,DAS,CORPORATE_FUNCTIONS,RAS,DN,MS,CAPABILITY_HUBS,
        Dated_On,FILE_DATE,left(File_Date,6) as Month_Key from kgsonedatadb.'''+hist_table)
    ...
    if table == "trusted_hist_impact_it_purchased_goods_consumption_data":
        sampleDF=spark.sql('''select HEADS,DEFINITION,UNITS,KGS_DATA,USD,FY,COMMENTS,Dated_On,FILE_DATE,left(File_Date,6) as Month_Key from kgsonedatadb.'''+hist_table)
    ...
    if table == "trusted_hist_impact_integrity_training_data":
    ...
    if table == "trusted_hist_impact_waste_management_data":
        sampleDF=spark.sql('''select LOCATION,FY,MONTH,FOOD_WASTE__KGS_,WATER_CONSUMPTION__K_LTRS_,PAPER_CONSUMPTION__KGS_,DRY_WASTE__KGS_,PANTRY_CONSUMABLES__INR_,HOUSEKEEPING_CONSUMABLES__INR_,RECYCLED_WASTE__KGS_,	OTHER_WASTE___NON_RECYCLED_WASTE__KGS_,TOTAL_WASTE__INCLUDES_RECYCLED_WASTE___KGS_,Dated_On,FILE_DATE,left(File_Date,6) as Month_Key from kgsonedatadb.'''+hist_table)
    ...
    if table == "trusted_hist_impact_access_card_data":
        sampleDF=spark.sql('''select CASE when (trim(AS_OF_DATE) == '' or AS_OF_DATE == '-') then NULL else AS_OF_DATE END as AS_OF_DATE,LOCATION,COST_CENTRE,EMPLOYEE_ID,EMPLOYEE_NAME,LEVEL,CASE when (trim(DATE) == '' or DATE == '-') then NULL else DATE END as DATE,DAY,BU,Dated_On,FILE_DATE,left(File_Date,8) as Month_Key from kgsonedatadb.'''+hist_table)
    ...
    if table == "trusted_hist_impact_admin_data":
       ...
       FILE_DATE,left(File_Date,6) as Month_Key from kgsonedatadb.'''+hist_table)
    ...
    if table == "trusted_hist_impact_car_lease_data":
    ...
    if table == "trusted_hist_impact_csr_data":
        sampleDF=spark.sql('''select EMPLOYEE_NAME,EMPLOYEE_ID,EMAIL_ID,LOCATION,DESIGNATION,SUB_FUNCTION_1,ACTIVITY_NAME,VIRTUAL_OR_NON_VIRTUAL,TYPE,HOURS,OUT_REACH_OF_BENEFICIERIES,LEVELWISE_MAPPING,MONTH,FY,Dated_On,FILE_DATE,left(File_Date,6) as Month_Key from kgsonedatadb.'''+hist_table)
    ...
    if table == "trusted_hist_impact_electricity_and_dg_data":
    ...
    if table == "trusted_hist_impact_idne_gender_target_vs_actuals":
        sampleDF=spark.sql('''select GOAL__KPI,TARGET_TYPE,TARGET_VALUE,TARGET_YEAR,CURRENT_YEAR_TARGET,CURRENT_YEAR_ACTUALS,CURRENT_YEAR,Dated_On,FILE_DATE,left(File_Date,6) as Month_Key from kgsonedatadb.'''+hist_table)  
    ...
    if table == "trusted_hist_impact_purchased_goods_and_services":
    ...
    if table == "trusted_hist_impact_air_travel_data":
        sampleDF=spark.sql('''select FY,INVOICE_MONTH,DOMESTIC__INTERNATIONAL,DK_DESCRIPTION,ENTITY,TRAVELER_NAME,	CLASS_OF_SERVICE_SEQUENCE,ITINERARY,ORIGIN_AIRPORT__PREDOMINANT_,DESTINATION_AIRPORT__PREDOMINANT_,`INVOICE_#`,AIR_NET_TICKET_AMOUNT,AIR_MILES,AIR_KMS,HAUL_MARKING__BASIS_AIR_KMS,CHARGEABLE__NON_CHARGEABLE,VOUCHER_TYPE,DESIGNATION,DEPARTMENT,TICKET_NO,CO2__EMISSION__IN_TONS,BUSINESS_UNIT,`FUNCTION`,SUB_FUNCTION,SERVICE_LINE, Dated_On,FILE_DATE,left(File_Date,6) as Month_Key  from kgsonedatadb.'''+hist_table)
    ...
    if table == "trusted_hist_impact_gps_data":
        sampleDF=spark.sql(''' SELECT YEAR,PARAMETERS ,KGS_OVERALL ,GDC ,KRC ,TAX ,CF ,CH ,CONSULTING ,DAS ,DN ,MS ,RA_C,Dated_On,FILE_DATE,left(File_Date,6) as Month_Key from kgsonedatadb.'''+hist_table)
...
if processName == "risk":
    if table == 'trusted_risk_kgs_india_mandatory_training':
        sampleDF=spark.sql('''select TRAINING, HR_ID, FULL_NAME,CASE when (trim(DATE_FIRST_HIRED) == '' or DATE_FIRST_HIRED == '-') then NULL else DATE_FIRST_HIRED END as DATE_FIRST_HIRED, PEOPLE_GROUP, JOB, CLIENT_GEOGRAPHY, EMPLOYEE_CATEGORY, FUNCTION, EMPLOYEE_SUBFUNCTION, EMPLOYEE_SUBFUNCTION_1, BU, COST_CENTRE, EMPLOYEE_EMAIL,CASE when (trim(ASSIGNED_DATE) == '' or ASSIGNED_DATE == '-') then NULL else ASSIGNED_DATE END as ASSIGNED_DATE,CASE when (trim(COMPLETION_DATE) == '' or COMPLETION_DATE == '-') then NULL else COMPLETION_DATE END as COMPLETION_DATE, COMPLETION_STATUS,CASE when (trim(DEADLINE) == '' or DEADLINE == '-') then NULL else DEADLINE END as DEADLINE, Dated_On, File_Date,left(File_Date,6) as Month_Key from kgsonedatadb.'''+hist_table)
    ...
    if table == 'trusted_risk_independence_violations':
        sampleDF=spark.sql('''select EMPLOYEE_ID,NAME,DATE_OF_JOINING,BUSINESS_UNIT,DESIGNATION,LINE_MANAGER,TYPE_OF_VIOLATION,CATEGORY_OF_VIOLATION,INVESTMENT_TYPE,RESTRICTION_DATE,PURCHASE_DATE,USER_ACQUISITION_DATE,KICS_ACQUISITION_DATE,ACTUAL_SALE_DATE,PURCHASE_PRICE,NUMBER_OF_SHARES,AMOUNT,ACTUAL_SALE_AMOUNT,NET_PROFIT,Dated_On,File_Date,left(File_Date,6) as Month_Key from kgsonedatadb.'''+hist_table)
    ...
    if table == 'trusted_risk_intune_compliance':
        sampleDF=spark.sql('''select DEVICE_NAME,COMPLIANCE,OS,OS_VERSION,PRIMARY_USER_EMAIL_ADDRESS,DEPARTMENT,ENTITY,DESIGNATION,INTUNE_REGISTERED,PHONE_NUMBER,PRIMARY_USER_DISPLAY_NAME,SUBSCRIBER_CARRIER,AZURE_AD_REGISTERED,DEVICE_ID,Dated_On,File_Date,left(File_Date,6) as Month_Key from kgsonedatadb.'''+hist_table)
    ...
    if table == 'trusted_risk_in_dl_allow_user_outbound':
        sampleDF=spark.sql('''select USERNAME, DISPLAYNAME, EMAILADDRESS, TITLE, DEPARTMENT, COMPANY, OFFICE, Dated_On, File_Date,left(File_Date,6) as Month_Key from kgsonedatadb.'''+hist_table)
    ...
    if table == 'trusted_risk_local_admin_users_kgs':
        sampleDF=spark.sql('''select HOSTNAME,NAME_OF_THE_LOCAL_GROUP,ACCOUNT_CONTAINED_WITHIN_THE_GROUP,ACCOUNT_TYPE,DOMAIN_FOR_ACCOUNT,TYPE_OF_ACCOUNT,RELIABILITY_OF_INFORMATION,Dated_On, File_Date,left(File_Date,6) as Month_Key from kgsonedatadb.'''+hist_table)
    ...
    if table == 'trusted_risk_laptop_recovery':
        sampleDF=spark.sql('''select EMPLOYEE_NUMBER, EMPLOYEE_NAME, EMAIL_ADDRESS, DESIGNATION, DEPARTMENT, ENTITY, LOCATION, RESIGNATION_INITIATED_DATE, APPROVED_LWD, AGEING, EMPLOYEE_CATEGORY, IT_CLEARANCE_DATE, IT_CLEARANCE_STATUS, IT_REMARKS, ADMIN_STATUS, ADMIN_REMARKS, HRSS_STATUS, HRSS_REMARKS, Dated_On, File_Date,left(File_Date,6) as Month_Key from kgsonedatadb.'''+hist_table)
    ...
    if table == 'trusted_risk_laptop_ageing':
        sampleDF=spark.sql('''select AGEING,SORT, Dated_On, File_Date,left(File_Date,6) as Month_Key from kgsonedatadb.'''+hist_table)
    ...
    if table == 'trusted_risk_usb_access':
        sampleDF=spark.sql('''select NAME,DESCRIPTION,EMAIL,Dated_On,File_Date,left(File_Date,6) as Month_Key from kgsonedatadb.'''+hist_table)
    ...
    if table == 'trusted_risk_gsoc':
        sampleDF=spark.sql('''select INCIDENT_ID, PRIORITY, THREAT_CATEGORY_GRUPING, THREAT_CATEGORY, DAY_OF_DATETIME_ASSIGNED, CONFIRMATION_SOURCE, CURRENT_STATUS, CLOSURE_COMMENT, DESCRIPTION, Dated_On, File_Date,left(File_Date,6) as Month_Key from kgsonedatadb.'''+hist_table)
    ...
    if table == 'trusted_risk_euda':
        sampleDF=spark.sql('''select SR__NO, EUDA_NAME, DESCRIPTION_OF_EUDA_PURPOSE_USE, ENTITY_NAME, FUNCTION, SUB_FUNCTION, CRITICALITY_ASSESSMENT_RATING, Dated_On, File_Date,left(File_Date,6) as Month_Key from kgsonedatadb.'''+hist_table)
    ...
    if table == 'trusted_risk_applications':
        sampleDF=spark.sql('''select ROW_LABELS, COUNT_OF_SOFTWARE_NAME, COMMENT, Dated_On, File_Date,left(File_Date,6) as Month_Key from kgsonedatadb.'''+hist_table)
    ...
    if table == 'trusted_risk_exit_report':
        sampleDF=spark.sql('''select EMPLOYEENUMBER, EMPLOYEENAME, EMAIL, DESIGNATION, ORGANIZATIONNAME, BU, ENTITY, LOCATION, LWD, PARTNERLWDAPPROVALDATE, ID_DISABLED_DATE_STAMP, SLA_MET___YES_NO, Dated_On, File_Date,left(File_Date,6) as Month_Key from kgsonedatadb.'''+hist_table)
    ...
    if table == 'trusted_risk_late_arrival_to_it_bin':
        sampleDF=spark.sql('''select EMPLOYEENUMBER, EMPLOYEENAME, EMAIL, DESIGNATION, ORGANIZATIONNAME, ENTITY, LOCATION, LWD, PARTNERLWDAPPROVALDATE, ID_DISABLED_DATE_STAMP, SLA_MET_YES_NO, REMARKS, BU, DELAY, Dated_On, File_Date,left(File_Date,6) as Month_Key from kgsonedatadb.'''+hist_table)
    ...
    if table == 'trusted_risk_m_bu':
        sampleDF=spark.sql('''select EMAIL_ADDRESS,PLEASE_PROVIDE_BU, Dated_On, File_Date,left(File_Date,6) as Month_Key from kgsonedatadb.'''+hist_table)
    ...
    if table == 'trusted_risk_m_job_name':
        sampleDF=spark.sql('''select EMPLOYEE_NUMBER,EMAIL_ADDRESS,POSITION,SHEET_NAME,JOB_NAME, Dated_On, File_Date,left(File_Date,6) as Month_Key from kgsonedatadb.'''+hist_table)
    ...
    if table == 'trusted_risk_m_applications':
        sampleDF=spark.sql('''select KGS_PURCHASED_CLOUD_APPLICATIONS,SNO,Dated_On, File_Date,left(File_Date,6) as Month_Key from kgsonedatadb.'''+hist_table)
    ...
    if table == 'trusted_risk_level':
        sampleDF=spark.sql('''select BU, POSITION, JOB, LEVELS, RANK, Dated_On, File_Date,left(File_Date,6) as Month_Key from kgsonedatadb.'''+hist_table)
    ...
    if table == 'trusted_risk_laptop_bu_mapping':
        sampleDF=spark.sql('''select DEPARTMENT,BU,Dated_On, File_Date,left(File_Date,6) as Month_Key from kgsonedatadb.'''+hist_table)
