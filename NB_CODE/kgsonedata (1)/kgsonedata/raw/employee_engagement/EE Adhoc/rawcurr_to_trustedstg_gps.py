...
# df_comp_yec = spark.sql("select * from kgsonedatadb.trusted_hist_compensation_yec where File_Date = '20230101'")
...
df_join_emp = df_emp_dump.join(df_comp_yec, df_comp_yec.EMP_ID ==  df_emp_dump.Employee_Number, 'left').select(df_emp_dump["*"],df_comp_yec["*"])
...
.withColumn("Full_time_or_Part_time", col("Emp_Category"))\
...
.withColumn("Is_a_High_Performer",when(col(rating_column).isin(list1), lit("Yes") ).otherwise(lit("No")))\
...
# df_leave=spark.sql("select Employee_Number, Leave_Start_Date, Leave_Type from kgsonedatadb.trusted_hist_headcount_leave_report where File_Date ='20230112'")
