...
currentDf= spark.read.format("delta").load(raw_curr_transport_url)    
...
currentDf=currentDf.select("*").withColumn("CC_Number",regexp_extract('CC', r'^(\d+)', 1)) \
...
casted_df=currentDf.select("*").withColumn("CC_Number",col("CC_Number").cast("int")) \
                               ...
                               .withColumn("Amount",f.regexp_replace(f.col("Amount"), "[),]", "")) \
