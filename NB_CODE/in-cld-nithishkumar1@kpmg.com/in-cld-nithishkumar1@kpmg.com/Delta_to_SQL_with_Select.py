...
if processName == "TA_post_hire":
    ...
    if table == "trusted_TA_post_hire_CWK":
        sampleDF=spark.sql('select Req__Identifier,Candidate_Identifier,Candidate_Name,Designation,Cost_Centre,BU,Geo,Recruiter_Name,Email,Hiring_Source,Project_Code,Start_Date,Joining_Location,Sub_Location,Location_Category,Exception_Approval_To_On_Board,ZONE__For_Delhi__NCR_,Phone_Number,Current_Address_With_landmark_where_Laptop_Can_be_Delivered,Candidate_Category__ROD_or_RPO_,Remarks,KGS_Email_ID,Status,Updated_DOJ,BGV_Reference_No_,BGV_Status,Report_Colour,Mandatory_Checks_Completed,All_Checks_Completed,Case_Remarks,Onboarding_Approval,Approval_Date,Final_Joining_Status,Dated_On,File_Date,left(File_Date,8) as Month_Key from kgsonedatadb.'+table)
...
if processName == "it":
    ...
    if table == "trusted_it_software_approval_request":
        ...
        sampleDF=spark.sql('''select Number,Item,Approval_Status,Software_Name,Publisher___OEM_Name,Requested_For,Usage_End_Date,License_Type,Project_Number__Rekonnect_,Entity,Function,Sub_Function,   Dated_On,FILE_DATE,left(File_Date,8) as Month_Key from kgsonedatadb.'''+table)
...
if processName == "admin":
    ...
    if table == "trusted_admin_xport_planned_vs_actuals":
        sampleDF=spark.sql('''select Location,Date,DEPARTMENT as BU,cast(Planned as int),cast(Actual as int),cast(NO_SHOWS as int) as Variance,BU2,Dated_On,FILE_DATE,left(File_Date,8) as Month_Key from kgsonedatadb.'''+hist_table)
    ...
    if table == "trusted_admin_xport_no_shows":
        sampleDF=spark.sql('''select LOCATION,DATE,EMPLOYEE_NUMBER,NAME,DEPARTMENT as TEAM,BU2,Dated_On,FILE_DATE,left(File_Date,8) as Month_Key from kgsonedatadb.'''+hist_table)
    ...
    if table == "trusted_admin_card_swipe_master_data":
        sampleDF=spark.sql('''select `LOCATION`,DEPARTMENT AS BU,PAYROLL_NUMBER AS EMPLOYEE_ID,NAME___REG_NUMBER AS EMPLOYEE_NAME,JOB_TITLE AS LEVEL,TRANSACTION_TIME AS DATE,BU2,Dated_On,FILE_DATE,left(File_Date,8) as Month_Key from kgsonedatadb.'''+hist_table)
    ...
    if table == "trusted_admin_booked_and_no_shows":
        sampleDF=spark.sql('''select `LOCATION`,BU,EMPLOYEE_ID,EMPLOYEE_NAME,STATUS2,FROM_DATE,STATUS,Dated_On,FILE_DATE,left(File_Date,8) as Month_Key from kgsonedatadb.'''+hist_table)
    ...
    if table == "trusted_admin_locationwise_holidaylist":
        sampleDF=spark.sql('''select Occasion,Date,Location,Dated_On,FILE_DATE,left(File_Date,8) as Month_Key from kgsonedatadb.'''+table)
    ...
    if table == "trusted_admin_employee_data":
        sampleDF=spark.sql('''select Emp_ID,Name,Email_Id,Username,Employee_Type,Designation____Grade,Geography,Function,Sub_Function,Service_Line,Department,Company_Name,Location,Mobile,FILE_DATE,left(File_Date,8) as Month_Key from kgsonedatadb.'''+table)
    ...
    if table == "trusted_admin_cwk_data":
        sampleDF=spark.sql('''select Emp_ID,Name,Email_Id,Username,Employee_Type,Designation___Grade,Geography,Function,Subfunction,Service_Line,Department,Company_Name,Location,Mobile,FILE_DATE,left(File_Date,8) as Month_Key from kgsonedatadb.'''+table)
    ...
    if table == "trusted_admin_loaned_data":
        sampleDF=spark.sql('''select Emp_ID,Name,Email_Id,Username,Employee_Type,Designation____Grade,Geography,Function,Subfunction,Service_Line,Department,Company_Name,Location,Mobile,FILE_DATE,left(File_Date,8) as Month_Key from kgsonedatadb.'''+table)
...
if processName == "compensation":
    if table == "trusted_compensation_employee_ratings":
        sampleDF=spark.sql('''select EMPLOYEE_NUMBER,NAME,EMAIL,EMPLOYEE_CATEGORY,BU_MAPPING,DEPARTMENT,RATING,FY,Dated_On,FILE_DATE,left(File_Date,8) as Month_Key from kgsonedatadb.'''+table)
