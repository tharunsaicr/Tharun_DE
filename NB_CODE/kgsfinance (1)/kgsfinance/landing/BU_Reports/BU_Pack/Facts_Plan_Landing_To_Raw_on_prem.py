...
df = spark.read.format("csv").option("header","true").option("delimiter",",").option("escape","\"").option("multiLine","true").option("escapeQuotes", "true").load(finance_landing_path_url+filePath)
...
#change Scenario field value to 'Plan' for all rows
df = df.withColumn("Scenario",lit("Plan"))
display(df)
