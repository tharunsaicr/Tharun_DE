...
# This NB handles Cost Actual & Forecast
...
# DBTITLE 1,Call connection module,common components module and  Unpivot module
...
currentDf=currentDf.withColumn('MMM',upper(substring(col("Month"),0,3))).withColumnRenamed("Amount","Value")

currentDf=currentDf.withColumn("Value",col("Value").cast('double')).withColumn("Calendar_Year",when(currentDf['MMM'].isin(month_list),(currentDf['Financial_Year'].cast('int'))-1).otherwise((currentDf['Financial_Year']).cast('int'))).withColumn('Month_Key',concat(col("Calendar_Year"),from_unixtime(unix_timestamp(col("MMM"),'MMM'),'MM')))
