...
display(hrmsUnionDataDf)

# COMMAND ----------

dailyAttendanceReportDf.display()

# COMMAND ----------

visitedWOBookingDf.display()
...
# Records where BU is available
currentDf_withoutNullBU = currentDf.filter((col("BU").isNotNull()) & (col("BU") != '-'))
currentDf_withoutNullBU.count()

# COMMAND ----------

# Records where BU is not available 
currentDf_withNullBU =  currentDf.filter((col("BU").isNull()) | (col("BU") == '-') )
currentDf_withNullBU =currentDf_withNullBU.drop('Cost_centre','BU')

# Before Join
currentDf_withNullBU.count()

# COMMAND ----------

# so trying to get Department from HRMS and then find BU from mapping file

currentDf_withNullBU = currentDf_withNullBU.join(hrmsUnionDataDf, (currentDf_withNullBU.EMPLOYEE_ID == hrmsUnionDataDf.HRMS_EMP_ID) & (currentDf_withNullBU.EMPLOYEE_NAME == hrmsUnionDataDf.HRMS_NAME),"left")

currentDf_withNullBU = currentDf_withNullBU.withColumn('DEPARTMENT', when((col('HRMS_DEPARTMENT').isNotNull()) & (col('HRMS_DEPARTMENT') != '-'), col('HRMS_DEPARTMENT')).otherwise(col('DEPARTMENT')))

# Get BU for Deparment that we have got from HRMS
currentDf_withNullBU = currentDf_withNullBU.join(buDf, upper(currentDf_withNullBU.DEPARTMENT)== upper(buDf.Cost_centre), "left")

currentDf_withNullBU = currentDf_withNullBU.select('LOCATION','DEPARTMENT','EMPLOYEE_ID','EMPLOYEE_NAME','STATUS2','FROM_DATE','STATUS','Cost_centre','BU')
...
currentDf = currentDf_withoutNullBU.union(currentDf_withNullBU)
