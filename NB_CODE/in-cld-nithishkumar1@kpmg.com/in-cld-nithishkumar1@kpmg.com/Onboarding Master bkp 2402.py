...
if tableName=='analytics':
    ...
    currentDf=currentDf.select("EMPLOYEE_NAME","PSID","DESIGNATION","LOCATION","DOJ","GENDER","INDUSTRY_GROUP_COST_CENTER","INDUSTRY","TYPE_OF_HIRE_FTE_FTS","CAMPUS_PROFESSIONAL","INDIA_EMAIL_ID","US_VDI_EMAIL_ID","File_Date","PERSONAL_EMAIL_ID","PM","PM_EMAIL_ID","BUDDY_NAME","BUDDY_EMAIL_ID")

    currentDf=currentDf.withColumnRenamed("EMPLOYEE_NAME","EMP_NAME") \
                       .withColumnRenamed("TYPE_OF_HIRE_FTE_FTS","EMP_CATEGORY") \
...
if tableName  in ("absconding","active_hc","attrition","attrition_pipeline","long_leave","fts","secondee","transfers"):
    ...
    case when (a.PSID=b.PS_ID or a.PSID=c.PS_ID or a.PSID=d.PSID or a.PSID=e.US_ID) and (a.PSID!=i.PS_ID ) and (a.PSID!=f.PS_ID) then 'Active'
...
# MAGIC %sql
# MAGIC select PSID,COUNT(*) from kgsonedatadb.trusted_hist_gdc_onboarding_master group by PSID  --and SERVICE_LINE='GTA'
...
spark.sql('''Update kgsonedatadb.trusted_hist_gdc_onboarding_master 
            SET CANDIDATES_STATUS= CASE 
                                        ...
                                        WHEN ((RISK_TEAM_HANDOVER_TO_HRBP is Null or RISK_TEAM_HANDOVER_TO_HRBP ="") and (TARGET_RISK_CLEARANCE_DATE>=GETDATE()))  then "Risk Team clearance pending(Scheduled)"

                                        WHEN ((RISK_TEAM_HANDOVER_TO_HRBP is Null or RISK_TEAM_HANDOVER_TO_HRBP ="") and (TARGET_RISK_CLEARANCE_DATE<GETDATE()))  then "Risk Team clearance pending(Delay)"
                                        ...
                                        WHEN (HRBP_HANDOVER_TO_RMT_TEAM is Null or HRBP_HANDOVER_TO_RMT_TEAM ="") and (RISK_TEAM_HANDOVER_TO_HRBP is Not Null or RISK_TEAM_HANDOVER_TO_HRBP !="") and (date_add(RISK_TEAM_HANDOVER_TO_HRBP,1)<GETDATE()) then "With HRBP(Delay)"
                                         ...
                                         WHEN (RMT_HANDOVER_TO_INDUSTRY is Null or RMT_HANDOVER_TO_INDUSTRY ="") and (HRBP_HANDOVER_TO_RMT_TEAM is Not Null or HRBP_HANDOVER_TO_RMT_TEAM !="") and (date_add(HRBP_HANDOVER_TO_RMT_TEAM,1)>=GETDATE()) then "With HRBP(Scheduled)"

                                        WHEN (RMT_HANDOVER_TO_INDUSTRY is Null or RMT_HANDOVER_TO_INDUSTRY ="") and (HRBP_HANDOVER_TO_RMT_TEAM is Not Null or HRBP_HANDOVER_TO_RMT_TEAM !="") and (date_add(HRBP_HANDOVER_TO_RMT_TEAM,1)<GETDATE()) then "With HRBP(Delay)"
