...
tableName = "talent_konnect_resignation_status_report_fortnight"
...
finaldf = spark.sql("select * from (select row_number() over(partition by employeenumber order by resignationdate desc) as tk_rownum,* from kgsonedatadb.raw_curr_headcount_talent_konnect_resignation_status_report_fortnight) where tk_rownum = 1")
