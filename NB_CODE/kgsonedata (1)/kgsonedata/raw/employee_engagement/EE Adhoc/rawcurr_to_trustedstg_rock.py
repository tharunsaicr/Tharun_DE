...
df = spark.sql("select * from kgsonedatadb.trusted_hist_headcount_employee_dump where File_Date = '20230228' and Entity = 'KGS'")

# print(df.count())
df_config_cc_bu = spark.sql("select distinct Cost_centre,BU from kgsonedatadb.config_cost_center_business_unit")

df = df.join(df_config_cc_bu,df.Cost_centre == df_config_cc_bu.Cost_centre,"left").select(df["*"],df_config_cc_bu["BU"])
...
currentDf=df.select("Employee_Number","Full_Name","Function","Sub_Function","Cost_centre","BU","Location","Position","Employee_Category","Gratuity_Date","Date_First_Hired","Email_Address","Company_Name","desired_LWD","Approved_LWD","RM_Employee_Name","Rock_Tenure","Amount","Tenure_Month","Tenure_Year","Tenure_as_on")
...
# currentDf.select("Gratuity_Date","Rock_Tenure").display()
