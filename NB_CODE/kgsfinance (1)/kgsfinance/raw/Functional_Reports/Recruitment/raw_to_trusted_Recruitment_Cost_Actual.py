...
# DBTITLE 1,Replace Column Values
current_df=read_df.withColumn('GEO_revised', F.when(read_df.GEO_revised == '#N/A', F.lit('N/A')).otherwise(f.col("GEO_revised"))) \
                  .withColumn('Entity', F.when(read_df.Entity == '#Missing', F.lit('0')).otherwise(f.col("Entity")))
for colname in current_df.columns:
    current_df = current_df.withColumn(colname, when(f.col(colname)=="#N/A","0").otherwise(f.col(colname)))

# COMMAND ----------

# DBTITLE 1,Changing Column DataTypes
#casting columns to required datatype
current_df=current_df.select("*").withColumn("Debit_Amt",f.col("Debit_Amt").cast("double")) \
...
# DBTITLE 1,Casting YTD and  USD columns
...
# DBTITLE 1,Rename to YTD Sep for BI
column_names=current_df.columns

pattern="^YTD(.*)"
for column_name in column_names:
    if column_name.startswith("YTD"):
        new_column_name="YTD"+"_"+"Sep"
        current_df=current_df.withColumnRenamed(column_name,new_column_name)
