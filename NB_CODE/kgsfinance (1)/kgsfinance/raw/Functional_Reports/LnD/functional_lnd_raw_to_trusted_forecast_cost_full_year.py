...
# Functional Reports- Load LnD Forecast Cost full year data from delta table raw_curr_fr_lnd_forecast_cost_full_year
raw_curr_lnd_forecast_cost_full_year = spark.read.table("kgsfinancedb.raw_curr_"+ReportName+"_"+ processName + "_" +tableName)
raw_curr_lnd_forecast_cost_full_year.count()

# COMMAND ----------

display(raw_curr_lnd_forecast_cost_full_year)

# COMMAND ----------

# DBTITLE 1,Unpivoting the month columns
df_transform_lnd_forecast_cost_full_year=unpivotdf(raw_curr_lnd_forecast_cost_full_year)
display(df_transform_lnd_forecast_cost_full_year)

# COMMAND ----------

#Renaming columns, changing datatype
Month_List=['OCT','NOV','DEC']
df_transform_lnd_forecast_cost_full_year=df_transform_lnd_forecast_cost_full_year.withColumn("CC_No",col("CC_No").cast("int"))\
...
df_transform_lnd_forecast_cost_full_year.write \
.mode("overwrite") \
.format("delta") \
