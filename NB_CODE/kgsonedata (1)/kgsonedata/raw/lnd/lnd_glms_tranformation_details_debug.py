...
def rename_columns(df,var):
    for c in df.columns:
        df=df.withColumnRenamed(c,var+c)
    return df
...
df_emp_details=spark.sql("select * from (select rank() over(partition by employee_number,file_date order by dated_on desc) as rank, * from kgsonedatadb.trusted_hist_headcount_monthly_employee_details A where Company_Name = 'KPMG Global Services Private Limited' and file_date = (select max(file_date) from kgsonedatadb.trusted_hist_headcount_monthly_employee_details B where B.Employee_Number=A.Employee_Number and to_date(File_Date,'yyyyMMdd') <= '"+str(convertedFileDate)+"'"+")) ed_hist where rank = 1").withColumn("Job_Name", split(col("Job_Name"), "\.").getItem(1))

# COMMAND ----------

display(df_emp_details)

# COMMAND ----------

# DBTITLE 1,Load data from  GLMS , Employee dump and Termination Dump 
#step 1 Read data from Payroll cut off file -->headcount monthly details
emp_details=spark.sql("select * from (select rank() over(partition by employee_number,file_date order by dated_on desc) as rank, * from kgsonedatadb.trusted_hist_headcount_monthly_employee_details A where Company_Name = 'KPMG Global Services Private Limited' and file_date = (select max(file_date) from kgsonedatadb.trusted_hist_headcount_monthly_employee_details B where B.Employee_Number=A.Employee_Number and to_date(File_Date,'yyyyMMdd') <= '"+str(convertedFileDate)+"'"+")) ed_hist where rank = 1").withColumn("Job_Name", split(col("Job_Name"), "\.").getItem(1))

print("df_emp_details ",emp_details.count())
display(emp_details.select("File_Date").distinct())

emp_details=emp_details.withColumn('row_num',row_number().over(Window.partitionBy(emp_details.Employee_Number).orderBy(emp_details.Employee_Number))).where((col('row_num')==1)).drop('row_num')

emp_details=rename_columns(emp_details,'Emp_details_')

#step 2 Load Employee Dump
emp_dump=spark.sql("select * from (select rank() over(partition by employee_number,file_date order by dated_on desc) as rank, * from kgsonedatadb.trusted_hist_headcount_employee_dump A where Entity = 'KGS' and file_date = (select max(file_date) from kgsonedatadb.trusted_hist_headcount_employee_dump B where B.Employee_Number=A.Employee_Number and to_date(File_Date,'yyyyMMdd') <= '"+str(convertedFileDate)+"'"+")) ed_hist where rank = 1").withColumn("Job_Name", split(col("Job_Name"), "\.").getItem(1))

print("df_emp_dump ",emp_dump.count())
display(emp_dump.select("File_Date").distinct())

emp_dump=emp_dump.withColumn('row_num',row_number().over(Window.partitionBy(emp_dump.Employee_Number).orderBy(emp_dump.Employee_Number))).where((col('row_num')==1)).drop('row_num')

emp_dump=rename_columns(emp_dump,'Emp_')
#step 3 load termination dump
termination_dump = spark.sql("select * from (select rank() over(partition by employee_number,file_date order by dated_on desc) as rank, * from kgsonedatadb.trusted_hist_headcount_termination_dump A where file_date = (select max(file_date) from kgsonedatadb.trusted_hist_headcount_termination_dump B where B.Employee_Number=A.Employee_Number and to_date(File_Date,'yyyyMMdd') <= '"+str(convertedFileDate)+"'"+")) td_hist where rank = 1").withColumn("Job_Name", split(col("Job_Name"), "\.").getItem(1))

termination_dump=termination_dump.withColumn('row_num',row_number().over(Window.partitionBy(termination_dump.Employee_Number).orderBy(termination_dump.Employee_Number))).filter(col('row_num')==1).drop('row_num')

print("df_termination_dump ",termination_dump.count())
termination_dump=rename_columns(termination_dump,'TD_')

# COMMAND ----------

# #display(emp_details)  # PK- Emp_details_Employee_Number
# #display(emp_dump)  #PK -Emp_Employee_Number

# # Count the data
# #emp_details.count() #24781
# #emp_dump.count() #27780

# # Distinct records
# #emp_details.select("Emp_details_Employee_Number").distinct().count() #24781
# #emp_dump.select("Emp_Employee_Number").distinct().count() #27780

# # Common employee records
# df=emp_details.join(emp_dump,emp_details.Emp_details_Employee_Number==emp_dump.Emp_Employee_Number, "inner")
# df.count()

# # Number of employees in employee dump who are not in payroll cut-off file = 9523
# df1=emp_dump.join(emp_details,emp_details.Emp_details_Employee_Number==emp_dump.Emp_Employee_Number, "left_anti")
# print(df1.count())
# display(df1)


# # Number of employees in payroll cut-off file who are not in employee dump = 6524
# # Category
# # Serving NP
# # Probation
# # any other
# df2=emp_details.join(emp_dump,emp_details.Emp_details_Employee_Number==emp_dump.Emp_Employee_Number, "left_anti")
# print(df2.count())
# display(df2)

# COMMAND ----------

# df_emp_details=df_emp_details.join(df_emp_dump, df_emp_details.Emp_details_Employee_Number == df_emp_dump.Emp_Employee_Number, 'left_anti').select(df_emp_details['*'])

# df_emp_details=df_emp_details.join(df_termination_dump, df_emp_details.Emp_details_Employee_Number == df_termination_dump.TD_Employee_Number, 'left_anti').select(df_emp_details["*"])

emp_dump = emp_dump.join(termination_dump, emp_dump.Emp_Employee_Number == termination_dump.TD_Employee_Number, 'left_anti').select(emp_dump["*"])

df_emp_dump=emp_dump.join(emp_details,emp_dump.Emp_Employee_Number==emp_details.Emp_details_Employee_Number, 'left_anti').select(emp_dump['*'])

termination_dump = termination_dump.join(emp_details, emp_details.Emp_details_Employee_Number == termination_dump.TD_Employee_Number, 'left_anti').select(termination_dump["*"])

df_termination_dump = termination_dump.join(df_emp_dump, df_emp_dump.Emp_Employee_Number == termination_dump.TD_Employee_Number, 'left_anti').select(termination_dump["*"])



print('emp_details ',emp_details.count())
...
df_level_wise=rename_columns(df_level_wise,'lw_')
...
#Join GLMS data with Employee Details, Employee Dump and Termination Dump\
    
df_join_lnd_emp_sl=df_raw_curr_lnd_glms.alias('glms').join(emp_details,df_raw_curr_lnd_glms.Local_HR_ID ==  emp_details.Emp_details_Employee_Number, 'left').select('glms.*','Emp_details_Employee_Number','Emp_details_Full_Name','Emp_details_Function','Emp_details_Employee_Subfunction','Emp_details_Employee_Subfunction_1','Emp_details_Cost_centre','Emp_details_Client_Geography','Emp_details_Location','Emp_details_Position','Emp_details_Job_Name','Emp_details_Email_Address','Emp_details_Gender','Emp_details_File_Date')
print("df_join_lnd_join_emp_details",df_join_lnd_emp_sl.count())

df_join_lnd_emp_sl=df_join_lnd_emp_sl.alias('glms_emp_details').join(df_emp_dump,df_join_lnd_emp_sl.Local_HR_ID ==  df_emp_dump.Emp_Employee_Number, 'left').select('glms_emp_details.*','Emp_Employee_Number','Emp_Full_Name','Emp_Function','Emp_Employee_Subfunction','Emp_Employee_Subfunction_1','Emp_Cost_centre','Emp_Business_Category','Emp_Client_Geography','Emp_Location','Emp_Position','Emp_Job_Name','Emp_Email_Address','Emp_Gender','Emp_File_Date')
print("df_join_lnd_join_ed",df_join_lnd_emp_sl.count())

df_join_lnd_emp_sl=df_join_lnd_emp_sl.alias('glms_emp_details_EmpDump').join(df_termination_dump,df_join_lnd_emp_sl.Local_HR_ID ==  df_termination_dump.TD_Employee_Number, 'left').select('glms_emp_details_EmpDump.*','TD_Employee_Number','TD_Full_Name','TD_Function','TD_Employee_Subfunction','TD_Employee_Subfunction_1','TD_Cost_Centre','TD_Business_Category','TD_Client_Geography','TD_Location','TD_Position','TD_Job_Name','TD_email_Address','TD_Gender','TD_File_Date')
...
# DBTITLE 1,Assign Position,Job_Name,Location,Gender from Payroll Cut-off file
df_join_lnd_emp_sl=df_join_lnd_emp_sl.withColumn('Position',df_join_lnd_emp_sl.Emp_details_Position)\
                                     .withColumn('Job_Name',df_join_lnd_emp_sl.Emp_details_Job_Name)\
                                     .withColumn('Location',df_join_lnd_emp_sl.Emp_details_Location)\
                                     .withColumn('Gender',df_join_lnd_emp_sl.Emp_details_Gender)\
                                     .withColumn('Cost_Centre',df_join_lnd_emp_sl.Emp_details_Cost_centre)\
                                     .withColumn('Geo',df_join_lnd_emp_sl.Emp_details_Client_Geography)

# COMMAND ----------

# For Position
df_join_lnd_emp_sl=df_join_lnd_emp_sl.withColumn('Position',when(col('Position').isNotNull(),col('Position'))\
    .when(col('Position').isNull()&col('Emp_Position').isNull() & col('TD_Position').isNotNull(),col('TD_Position'))\
    .when(col('Position').isNull()&col('Emp_Position').isNotNull() & col('TD_Position').isNull(),col('Emp_Position'))\
    .when(col('Position').isNull()&col('Emp_Position').isNotNull() & col('TD_Position').isNotNull(),col('Emp_Position'))\
    .otherwise(None))

# COMMAND ----------

#For Job Name
df_join_lnd_emp_sl=df_join_lnd_emp_sl.withColumn('Job_Name',when(col('Job_Name').isNotNull(),col('Job_Name'))\
    .when(col('Job_Name').isNull()&col('Emp_Job_Name').isNull() & col('TD_Job_Name').isNotNull(),col('TD_Job_Name'))\
    .when(col('Job_Name').isNull()&col('Emp_Job_Name').isNotNull() & col('TD_Job_Name').isNull(),col('Emp_Job_Name'))\
    .when(col('Job_Name').isNull()&col('Emp_Job_Name').isNotNull() & col('TD_Job_Name').isNotNull(),col('Emp_Job_Name'))\
    .otherwise(None))

# COMMAND ----------

#For Location
df_join_lnd_emp_sl=df_join_lnd_emp_sl.withColumn('Location',when(col('Location').isNotNull(),col('Location'))\
    .when(col('Location').isNull()&col('Emp_Location').isNull() & col('TD_Location').isNotNull(),col('TD_Location'))\
    .when(col('Location').isNull()&col('Emp_Location').isNotNull() & col('TD_Location').isNull(),col('Emp_Location'))\
    .when(col('Location').isNull()&col('Emp_Location').isNotNull() & col('TD_Location').isNotNull(),col('Emp_Location'))\
    .otherwise(None))

# COMMAND ----------

#For Gender
df_join_lnd_emp_sl=df_join_lnd_emp_sl.withColumn('Gender',when(col('Gender').isNotNull(),col('Gender'))\
    .when(col('Gender').isNull()&col('Emp_Gender').isNull() & col('TD_Gender').isNotNull(),col('TD_Gender'))\
    .when(col('Gender').isNull()&col('Emp_Gender').isNotNull() & col('TD_Gender').isNull(),col('Emp_Gender'))\
    .when(col('Gender').isNull()&col('Emp_Gender').isNotNull() & col('TD_Gender').isNotNull(),col('Emp_Gender'))\
    .otherwise(None))

# COMMAND ----------

# display(df_join_lnd_emp_sl.select('Gender','Emp_details_Gender','Emp_Gender','TD_Gender').where(col('Gender').isNull()).distinct())
...
display(df_join_lnd_joined.select('Position','Job_Name','lw_Mapping').distinct())
display(df_join_lnd_joined.select('Item_Title').where(col('TrainingCategory').isNull()).distinct())
...
df_join_lnd_joined=df_join_lnd_joined.withColumn('Cost_Centre',when(col('Cost_Centre').isNotNull(),col('Cost_Centre'))\
.when(col('Cost_Centre').isNull()&col('Emp_Cost_centre').isNotNull() & col('TD_Cost_Centre').isNull(),col('Emp_Cost_centre'))\
.when(col('Cost_Centre').isNull()&col('TD_Cost_Centre').isNotNull() & col('Emp_Cost_centre').isNull(),col('TD_Cost_Centre'))\
.when(col('Cost_Centre').isNull()&col('TD_Cost_Centre').isNotNull() & col('Emp_Cost_centre').isNotNull(),col('Emp_Cost_Centre'))\
.otherwise(None))

# COMMAND ----------

# display(df_join_lnd_joined.select('local_HR_Id','Email_Address__User_','Cost_Centre','Emp_details_Cost_centre','Emp_Cost_centre','TD_Cost_Centre').where(col('Cost_Centre').isNull()).distinct())
...
df_join_lnd_joined=df_join_lnd_joined.withColumn('Geo',when(col('Geo').isNotNull(),col('Geo'))\
.when(col('Geo').isNull()&col('Emp_Client_Geography').isNotNull() & col('TD_Client_Geography').isNull(),col('Emp_Client_Geography'))\
.when(col('Geo').isNull()&col('TD_Client_Geography').isNotNull() & col('Emp_Client_Geography').isNull(),col('TD_Client_Geography'))\
.when(col('Geo').isNull()&col('TD_Client_Geography').isNotNull() & col('Emp_Client_Geography').isNotNull(),col('Emp_Client_Geography'))\
.otherwise(None))
...
display(df_join_lnd_joined.agg(sum(col('CPD_CPE_Hours_Awarded'))))
...
display(df_join_lnd_joined.groupBy("BU").agg(sum(col('CPD_CPE_Hours_Awarded'))))
...
display(df_lnd_transformed.select('Item_Type','Item_Type_Category').distinct())
# .withColumn("Item_Type_Category",when(col("Item_Type") == "ILT","In-person").when(col("Item_Type") == "VLT","Virtual").when(col("Item_Type") == "HYBRID","Virtual").when(col("Item_Type") == "EXT","Virtual").when(col("Item_Type") == "CONF","Virtual").otherwise("Self Paced"))\

# COMMAND ----------

## Add Code to Load Bad records in another database table
df_lnd_transformed=df_lnd_transformed.withColumn('Emp_TD_Employee_Number',df_lnd_transformed.Emp_details_Employee_Number)\
.withColumn('Emp_TD_Email_Address',df_lnd_transformed.Emp_details_Email_Address)

finalDf=df_lnd_transformed.withColumn('Emp_TD_Employee_Number',when(col('Emp_TD_Employee_Number').isNotNull(),col('Emp_TD_Employee_Number'))\
    .when(col('Emp_TD_Employee_Number').isNull()&col('Emp_Employee_Number').isNull() & col('TD_Employee_Number').isNotNull(),col('TD_Employee_Number'))\
    .when(col('Emp_TD_Employee_Number').isNull()&col('Emp_Employee_Number').isNotNull() & col('TD_Employee_Number').isNull(),col('Emp_Employee_Number'))\
    .when(col('Emp_TD_Employee_Number').isNull()&col('Emp_Employee_Number').isNotNull() & col('TD_Employee_Number').isNotNull(),col('Emp_Employee_Number'))\
    .otherwise(None))\
    .withColumn('Emp_TD_Email_Address',when(col('Emp_TD_Email_Address').isNotNull(),col('Emp_TD_Email_Address'))\
    .when(col('Emp_TD_Email_Address').isNull()&col('Emp_Email_Address').isNull() & col('TD_email_Address').isNotNull(),col('TD_email_Address'))\
    .when(col('Emp_TD_Email_Address').isNull()&col('Emp_Email_Address').isNotNull() & col('TD_email_Address').isNull(),col('Emp_Email_Address'))\
    .when(col('Emp_TD_Email_Address').isNull()&col('Emp_Email_Address').isNotNull() & col('TD_email_Address').isNotNull(),col('Emp_Email_Address'))\
    .otherwise(None))
...
drop_cols=['Emp_details_Employee_Number','Emp_details_Full_Name','Emp_details_Function','Emp_details_Employee_Subfunction','Emp_details_Employee_Subfunction_1','Emp_details_Cost_centre','Emp_details_Client_Geography','Emp_details_Location','Emp_details_Position','Emp_details_Job_Name','Emp_details_Email_Address','Emp_details_Gender','Emp_Employee_Number','Emp_Full_Name','Emp_Function','Emp_Employee_Subfunction','Emp_Employee_Subfunction_1','Emp_Cost_centre','Emp_Business_Category','Emp_Client_Geography','Emp_Location','Emp_Position','Emp_Job_Name','Emp_Email_Address','Emp_Gender','TD_Employee_Number','TD_Full_Name','TD_Function','TD_Employee_Subfunction','TD_Employee_Subfunction_1','TD_Cost_Centre','TD_Business_Category','TD_Client_Geography','TD_Location','TD_Position','TD_Job_Name','TD_email_Address','TD_Location','TD_Position','TD_Gender','RM_Email_Address','RM_Cost_Centre','RM_Client_Geography']
