...
if tableName=='onboarding_master':
    columnlist=["BGV_CLEAR_DATE","LND_HANDOVER_TO_RISK_TEAM","HRBP_HANDOVER_TO_RMT_TEAM","RMT_HANDOVER_TO_INDUSTRY","LAPTOP_REACHED_ON","TIME_TAKEN_TO_RECEIVE_LND_CLEARANCE_DATE_FROM_LAPTOP_RECEIVED_DATE"]
    for column in columnlist:
        currentDf=currentDf.withColumn(column,when((col(column).isNull()|(col(column)=="null")),"-").otherwise(col(column)))
...
processList = ['admin']
tableNameList = ['booked_and_no_shows','card_swipe_master_data','xport_no_shows','xport_planned_vs_actuals']
...
if len(fileDateColumnList) > 0:
    ...
    elif (processName.lower() in processList) and (tableName.lower() in tableNameList):
        print('For selective Admin Tables as FileDate is in YYYYMMDD format no need to call UDF')
...
if  (processName.lower() == 'impact') & (tableName =='sick_wellbeing_data' or tableName =='wlb_attrition_data') & (('file_date').upper() in currentDf.columns):
    print("tableName =='sick_wellbeing_data' or tableName =='wlb_attrition_data'")
...
processList = ['admin','gdc']
tableNameList = ['booked_and_no_shows','card_swipe_master_data','xport_no_shows','xport_planned_vs_actuals','onboarding_master']

if (processName.lower() in processList) and (tableName.lower() in tableNameList) :
    layerName = 'raw_adhoc'
    dbutils.notebook.run("/kgsonedata/raw/Data_Cleanup",6000, {'DeltaTableName':tableName, 'ProcessName':processName, 'LayerName':layerName})
