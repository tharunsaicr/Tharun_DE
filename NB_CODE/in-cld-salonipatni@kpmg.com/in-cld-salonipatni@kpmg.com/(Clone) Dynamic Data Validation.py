...
config_table_query = f"SELECT a.Process_name,a.File_Name,b.SourceFileName,b.SourceFormat FROM [dbo].[config_table] a left join [dbo].[config_table_excel_to_csv] b  on replace(a.File_Name,'_',' ')=replace(b.TargetFileName,'_',' ') and replace(a.Process_name,'_',' ')=replace(b.ProcessName,'_',' ')  where a.Process_name in ('impact','csr') and a.File_Name not in ('glms_details','kva_details')" 
...
display(summary_df.filter(summary_df['Process Name'] =='impact'))

# COMMAND ----------

display(summary_df.filter(summary_df['Process Name'] =='csr'))
