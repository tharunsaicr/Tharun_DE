...
if(processName.upper()=='COMPENSATION'):
    currentDf.write \
    .mode("overwrite") \
    .format("delta") \
    .option("overwriteSchema", "True") \
    .option("path",isolated_trusted_curr_savepath_url+processName+"/"+tableName) \
...
if(processName.upper()!='GDC'):    
    dbutils.notebook.run("/kgsonedata/trusted/Delta_to_SQL_with_Select",6000, {'DeltaTableName':tableName, 'ProcessName':processName})

# COMMAND ----------

if(processName.upper()=='GDC'):    
    dbutils.notebook.run("/kgsonedata/trusted/Delta_to_SQL_GDC_adhoc",6000, {'DeltaTableName':tableName, 'ProcessName':processName})