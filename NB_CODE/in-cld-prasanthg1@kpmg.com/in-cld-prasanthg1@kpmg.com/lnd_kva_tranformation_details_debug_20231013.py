...
df_raw_curr_lnd_kva = spark.sql("select * from kgsonedatadb.raw_curr_lnd_kva_details where Employee_Id = '134208'")
print("raw kva count",df_raw_curr_lnd_kva.count())
...
#print("after raw kva",df_raw_curr_lnd_kva.count())

# #Load Employee Dump data
# df_emp_sl=spark.read.table("kgsonedatadb.employee_golden_data").withColumn("Job_Name", split(col("Job_Name"), "\.").getItem(1))
# print("before df emp dump sl",df_emp_sl.count())

# #exclude Headcount_Status not in('Maternity_Cases','Sabbatical')
# df_emp_sl=df_emp_sl.filter((col('Headcount_Status') != 'Maternity_Cases') & (col('Headcount_Status') != 'Sabbatical' ) )
# print("after df emp dump sl",df_emp_sl.count())
# df_emp_sl=df_emp_sl.drop("Dated_On")
# df_emp_sl=rename_emp_columns(df_emp_sl)
...
display(df_lnd_kva_col_updated)
...
#df_join_lnd_emp_sl_lw.select('lw_Mapping').distinct().display()
...
# .option("path", trusted_stg_lnd_url+"kva_details") \
# .option("compression","snappy") \
# .saveAsTable("kgsonedatadb.trusted_stg_lnd_kva_details")

# COMMAND ----------

# DBTITLE 1,Load data to final trusted table
# dbutils.notebook.run("/kgsonedata/trusted/trustedstg_to_trusted_load",6000,{'DeltaTableName':'kva_details','ProcessName':'lnd'})
