...
df_raw_curr_lnd_glms = spark.read.table("kgsonedatadb.test_raw_curr_lnd_glms_details")
...
display(df_raw_curr_lnd_glms.select("File_Date").distinct())
...
# alphadf= df_raw_curr_lnd_glms.select('*').filter(df_raw_curr_lnd_glms.Item_ID.rlike(".*[a-zA-Z].*") & col("item_id").rlike(".*[0-9].*"))
# display(alphadf)
...
print(df_config_cost_center_business_unit.count())
display(df_config_cost_center_business_unit)
...
df_join_lnd_emp_sl=df_join_lnd_emp_sl.alias('glms_EmpDump').join(df_termination_dump,df_join_lnd_emp_sl.Local_HR_ID ==  df_termination_dump.TD_Employee_Number, 'left').select('glms_EmpDump.*','TD_Employee_Number','TD_Employee_Name','TD_Function','TD_Sub_Function','TD_Cost_Centre','TD_Business_Category','TD_Client_Geography','TD_Location','TD_Position','TD_Job_Name','TD_email_Address','TD_File_Date')
...
# df_join_lnd_emp_sl_lw.select("Position","Job_Name").display()
...
display(badDf.agg(sum(col('CPD_CPE_Hours_Awarded'))))
...
.option("path", trusted_stg_lnd_url+"glms_details_test") \
.option("compression","snappy") \
.saveAsTable("kgsonedatadb.test_trusted_stg_lnd_glms_details")

# COMMAND ----------

# DBTITLE 1,Load data to final trusted table
dbutils.notebook.run("/Users/in-cld-tharunsaicr@kpmg.com/LnD Scripts/(Clone) trustedstg_to_trusted_load",6000,{'DeltaTableName':'glms_details','ProcessName':'lnd'})
...
# MAGIC %sql
# MAGIC --delete from kgsonedatadb.test_trusted_stg_lnd_kva_details;
# MAGIC --delete from kgsonedatadb.test_trusted_hist_lnd_kva_details;
# MAGIC --delete from kgsonedatadb.test_trusted_lnd_kva_details;
# MAGIC --delete from kgsonedatadb.test_trusted_hist_lnd_glms_kva_details where File_Type = 'KVA';
# MAGIC
# MAGIC -- select count(1) from kgsonedatadb.test_trusted_stg_lnd_glms_details;
# MAGIC -- select count(1) from kgsonedatadb.test_trusted_hist_lnd_glms_details;
# MAGIC -- select count(1) from kgsonedatadb.test_trusted_lnd_glms_details;
# MAGIC -- select count(1) from kgsonedatadb.test_trusted_hist_lnd_glms_kva_details where File_Type = 'GLMS';
# MAGIC
# MAGIC -- 116045
# MAGIC
# MAGIC