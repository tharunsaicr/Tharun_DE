...
currentDf = spark.sql("select * from kgsonedatadb.raw_hist_admin_transaction_report where File_Date = '20240330' and Dated_On = (select max(Dated_On) from kgsonedatadb.raw_hist_admin_transaction_report where File_Date = '20240330')")
display(currentDf)
...
if(tableName == 'transaction_report' and processName =='admin'):
    
    currentDf = currentDf.withColumn("TRANSACTION_TIME", from_unixtime(unix_timestamp("TRANSACTION_TIME",'MM/dd/yyyy hh:mm:ss aa'),"yyyy-MM-dd'T'HH:mm:ss.SSSZ"))
      

    currentDf = currentDf.withColumn("File_Date", regexp_replace(to_date('TRANSACTION_TIME'),'-',''))
    display(currentDf) 
