...
currentDf=currentDf.withColumn("Dated_On", to_timestamp(lit(currentdatetime)))\
 .withColumn("MMM",upper(F.date_format("Invoice_Date","MMM")))\
 .withColumn("Calendar_Year",F.year("Invoice_Date"))\
.withColumn("Financial_Year",when(f.col("MMM").isin(month_list),f.col("Calendar_Year").cast('int')+1).otherwise(f.col("Calendar_Year").cast('int')))\
