...
.option("path",config_path_url+"/"+tableName+"_test") \
.option("compression","snappy") \
.saveAsTable("kgsonedatadb.test_"+ processName + "_" +tableName)
...
if tableName == 'cc_bu_sl':
    ...
    currentDf = spark.sql("select distinct Cost_centre,Final_BU as BU,File_Date, Final_BU,Dated_On from kgsonedatadb.test_config_cc_bu_sl")

    currentDf.write \
    .mode("overwrite") \
    .format("delta") \
    .option("overwriteSchema", "True") \
    .option("path",config_path_url+"/"+"cost_center_business_unit_test") \
    .option("compression","snappy") \
    .saveAsTable("kgsonedatadb.test_"+ processName + "_" +"cost_center_business_unit")

    # currentDf.write \
    # .mode("append") \
    # .format("delta") \
    # .option("mergeschema", "True") \
    # .option("path",config_hist_path_url+"/"+"cost_center_business_unit_test") \
    # .option("compression","snappy") \
    # .saveAsTable("kgsonedatadb.test_"+ processName + "_hist_" +"cost_center_business_unit")
