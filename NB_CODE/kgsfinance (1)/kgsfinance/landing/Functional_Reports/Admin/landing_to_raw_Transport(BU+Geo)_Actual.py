...
#read the csv file
currentDf =spark.read.format("csv").option("header","true").option("negativeInf","True").option("delimiter",",").option("escape","\"").option("multiLine","true").option("escapeQuotes", "true").load(finance_landing_path_url+filePath)
...
#derive month,year and Financial year column from period

final_df=final_df.withColumn("Month",upper(substring('Period', 1,3)))\
                .withColumn("Calendar_Year",substring('Period',-4,4))
# display(final_df)
month_list=['oct','nov','dec']
load_df=final_df.withColumn('Financial_Year',when(lower(final_df['Month']).isin(month_list),final_df['Calendar_Year'].cast('int')+1).otherwise(final_df['Calendar_Year'].cast('int'))).withColumn("Dated_On", to_timestamp(lit(currentdatetime)))
display(load_df)

# COMMAND ----------

load_df=load_df.withColumn("Month_Key",concat(col("Calendar_Year"),from_unixtime(unix_timestamp(col("Month"),'MMM'),'MM')))
display(load_df)

# COMMAND ----------

load_df=load_df.drop(col("Period_1"))
