...
config_table_query = f"SELECT a.Process_name,a.File_Name,b.SourceFileName,b.SourceFormat FROM [dbo].[config_table] a left join [dbo].[config_table_excel_to_csv] b  on replace(a.File_Name,'_',' ')=replace(b.TargetFileName,'_',' ') and replace(a.Process_name,'_',' ')=replace(b.ProcessName,'_',' ')  where a.Process_name in ('jml','talent_acquisition','employee_engagement','headcount','lnd','it','headcount_monthly','BGV')" 
...
for row in config_table_df.collect():
    print(row["Process_name"] ," ", row['File_Name']," ",row["SourceFileName"])
...
# adls_path=f"/mnt/landinglayermount/pending/summary_report/{current_date}/{file_name}"
