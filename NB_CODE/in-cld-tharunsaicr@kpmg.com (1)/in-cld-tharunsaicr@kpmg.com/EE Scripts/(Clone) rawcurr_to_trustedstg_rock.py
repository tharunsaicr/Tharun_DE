...
import pytz
currdate=currentdatetime= datetime.now(pytz.timezone('Asia/Kolkata')).strftime('%Y-%m-%d')
print(currdate)
currentDf=currentDf.filter(~(coalesce(col('Approved_LWD').cast(DateType()),lit('9999-12-31')) < currdate))
# currentDf=currentDf.filter(col('Approved_LWD').isNull()
...
display(currentDf.select('Approved_LWD').distinct())
