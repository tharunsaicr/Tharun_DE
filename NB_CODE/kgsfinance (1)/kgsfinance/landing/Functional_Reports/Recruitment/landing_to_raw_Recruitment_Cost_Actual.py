...
drop_num_df=currentDf.select(currentDf.colRegex("`(\d)`"))
...
final_df=final_df.withColumn("Month",upper(substring('Period', 1,3)))\
                   .withColumn("Calendar_Year",substring('Period',5,4))
# display(final_df)
month_list=['oct','nov','dec']
load_df=final_df.withColumn('Financial_Year',when(lower(final_df['Month']).isin(month_list),final_df['Calendar_Year'].cast('int')+1).otherwise(final_df['Calendar_Year'].cast('int'))).withColumn("Dated_On", lit(currentdatetime))
display(load_df)
...
load_df=load_df.withColumn("Dated_On", to_timestamp(lit(currentdatetime)))
display(load_df)
