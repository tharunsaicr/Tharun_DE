...
if processName == "admin":
    ...
    if table == "trusted_admin_card_swipe_master_data":
        sampleDF=spark.sql('''select `LOCATION`,DEPARTMENT AS BU,PAYROLL_NUM AS EMPLOYEE_ID,NAME___REG_NUM AS EMPLOYEE_NAME,JOB_TITLE AS LEVEL,TRANSACTION_TIME AS DATE,BU2,Dated_On,FILE_DATE,left(File_Date,8) as Month_Key from kgsonedatadb.'''+table)
