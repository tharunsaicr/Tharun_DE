...
if processName == "compensation_non_sensitive":
    if table == "trusted_compensation_non_sensitive_qualification_dump":
        sampleDF=spark.sql('''select EMPLOYEE_NUMBER as EMP_ID,NAME as FULL_NAME,Post_Graduation___Highest_Qualification,GRAD,OTHERS,Dated_On,File_Date,left(File_Date,6) as Month_Key from kgsonedatadb.'''+table)
        # sampleDF=spark.sql('''select *,left(File_Date,6) as Month_Key from kgsonedatadb.'''+table)
        sampleDF.write \
        .format("jdbc") \
        .mode("append") \
