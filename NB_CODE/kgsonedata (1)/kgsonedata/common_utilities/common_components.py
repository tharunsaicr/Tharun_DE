...
# DBTITLE 1,Add Prefix to Column Names
def rename_prefix_columns(prefix,df):
...
#to replace the null values as '-'
def replacenull(df1):
    for colname in df1.columns:
        df1 = df1.withColumn(colname, when(col(colname).isNull(),lit('-')).otherwise(col(colname)))
    return df1