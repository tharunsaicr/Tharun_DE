...
# COMMAND ----------
    ...
    # orderCol = "Employee_Name"
    # w = Window.partitionBy(columnName).orderBy(columnName)
    # currentDf = currentDf.withColumn("Employee_Rank",row_number().over(w))

    # # display(currentDf.select("Employee_Rank").distinct())
    # display(currentDf)
...
for columnName in currentDf.columns:
    ...
    if (columnName in primaryKeyColumnList):
        ...
        w = Window.partitionBy(columnName).orderBy(columnName)
        currentDf = currentDf.withColumn(columnName+"_Rank",row_number().over(w))
        ...
        currentDf = currentDf.withColumn("Is_Primary_Duplicate_Record",when((col(columnName).isin(duplicateIdList)) & ((col(columnName).isNotNull()) & (col(columnName) != '') & (col(columnName) != '-')),"PK Duplicate Record")\
...
for columnName in currentDf.columns:
    ...
    if (columnName in primaryKeyColumnList):
        ...
        goodDf = currentDf.withColumn(columnName,when(upper(currentDf.Is_Primary_Key_Null) == 'PK IS NULL',concat(lit('NULL_'),lit(randomNumber(col(columnName))),lit('_'),lit(monotonically_increasing_id())))\
...
# df1 = goodDf.withColumn("MonoId",concat(lit('NULL_'),lit(randomNumber(col(columnName))),lit('_'),lit(monotonically_increasing_id())))

# df1 = goodDf.withColumn("MonoPKId",when(upper(currentDf.Is_Primary_Key_Null) == 'PK IS NULL',concat(lit('NULL_'),lit(randomNumber(col(columnName))),lit('_'),lit(monotonically_increasing_id()))).otherwise(col(columnName)))


# df1 = goodDf.withColumn("MonoPKId",when(((upper(currentDf.Is_Primary_Key_Null) == 'PK IS NULL') & (upper(currentDf.Is_Primary_Duplicate_Record) == 'PK DUPLICATE RECORD')),concat(lit('NULL_'),lit(randomNumber(col(columnName))),lit('_'),lit(monotonically_increasing_id()))).otherwise(col(columnName)))
