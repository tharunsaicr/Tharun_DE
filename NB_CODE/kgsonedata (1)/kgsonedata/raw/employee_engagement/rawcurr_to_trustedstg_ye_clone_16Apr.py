...
# MAGIC %sql
# MAGIC select * from kgsonedatadb.trusted_hist_compensation_yec where EMPLOYEE_NUMBER=='51668' order by File_Date desc,Dated_On desc
...
# display(df_com_yec.where(df_com_yec["Emp_ID"]=='97993'))
...
df_qual_dump = spark.sql("select qual_dump.EMP_ID as Qual_EMP_ID, qual_dump.Post_Graduation___Highest_Qualification as Post_Graduation_Highest_Qualification, qual_dump.GRAD as GRAD from (select row_number() over(partition by EMP_ID order by File_Date desc, Dated_On desc) as Rank_Grad,* from kgsonedatadb.trusted_hist_compensation_non_sensitive_qualification_dump where EMP_ID is not null) qual_dump where qual_dump.Rank_Grad = 1")
...
df_join_emp.where(df_join_emp.Employee_Number=='98952').display()
# .select('Pitched_at__Current_Positioning_','Pitching_Year')
# select * from kgsonedatadb.trusted_hist_compensation_yec where EMPLOYEE_NUMBER=='51668' order by File_Date desc,Dated_On desc
...
df_encore = spark.sql("select * from (select row_number() over(partition by Emp_No order by File_Date desc, Dated_On desc) rank_Encore,* from kgsonedatadb.trusted_hist_employee_engagement_encore_output where category not in ('Appreciation Month-Influencer','Festive Gift') and category not like '%Service Anniversary Award%' and Year = '"+fileYear_FY+"'"+") A where rank_Encore = 1")
