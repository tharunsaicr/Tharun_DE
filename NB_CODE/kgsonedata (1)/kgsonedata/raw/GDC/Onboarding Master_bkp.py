...
if tableName=='analytics':
    ...
    currentDf=currentDf.select("EMPLOYEE_NAME","PSID","DESIGNATION","LOCATION","DOJ","GENDER","INDUSTRY_GROUP_COST_CENTER","INDUSTRY","TYPE_OF_HIRE_FTE_FTS_SUB_CON","CAMPUS_PROFESSIONAL","INDIA_EMAIL_ID","US_VDI_EMAIL_ID","File_Date","PERSONAL_EMAIL_ID","PM","PM_EMAIL_ID","BUDDY_NAME","BUDDY_EMAIL_ID")
...
if tableName  in ("absconding","active_hc","attrition","attrition_pipeline","long_leave","fts","secondee","transfers"):
    ...
    case when (a.PSID=b.PS_ID or a.PSID=c.PS_ID or a.PSID=d.PSID or a.PSID=e.US_ID) then 'Active'
...
if tableName  in ("absconding","active_hc","attrition","attrition_pipeline","long_leave","fts","secondee","transfers"):
    ...
    ON a.PSID=b.PSID 
    WHEN MATCHED  THEN  
    UPDATE SET a.EMP_ID=b.Final_KPMG_ID,a.Joiners_Status=b.Candidate_Status,a.INDUSTRY_GROUP_COST_CENTER=b.HC_Cost_Center,a.UG=b.UG,a.PG=b.PG,a.HIGHEST_QUALIFICATION=b.HIGHEST_QUALIFICATION''')
...
# MAGIC %sql
# MAGIC select * from kgsonedatadb.trusted_hist_gdc_onboarding_master  where doj='2023-11-28' --and SERVICE_LINE='GTA'
...
if tableName=="bgv_check":
    spark.sql('''MERGE INTO kgsonedatadb.trusted_hist_gdc_onboarding_master a 
               ...
               ON a.EMP_ID=b.EMPLOYEE_ID  and a.EMP_ID is NOT NULL
               WHEN MATCHED  THEN  
...
if tableName=="newjoiners_status":
    spark.sql('''MERGE INTO kgsonedatadb.trusted_hist_gdc_onboarding_master a 
               USING  kgsonedatadb.trusted_gdc_newjoiners_status b
               ON a.PSID=b.PSID 
               WHEN MATCHED  THEN  
                UPDATE SET a.ATTENDED_GENESIS_KBS=b.GENESIS_DATE,a.NO_OF_COURSES_PENDING=case when b.TOTAL_PNG_COURSE==-1 then 0 else b.TOTAL_PNG_COURSE end''')
...
if tableName=='BCP':
    spark.sql('''MERGE INTO kgsonedatadb.trusted_hist_gdc_onboarding_master a 
               ...
               ON a.PSID=b.PSID   
               WHEN MATCHED  THEN  
                UPDATE SET a.BCP="BCP access reactivated"''')
...
if tableName=='hrbp_handover':
    spark.sql('''MERGE INTO kgsonedatadb.trusted_hist_gdc_onboarding_master a 
               USING  kgsonedatadb.trusted_gdc_hrbp_handover b
               ON a.PSID=b.PSID   
               WHEN MATCHED  THEN  
                UPDATE SET a.HRBP_HANDOVER_TO_RMT_TEAM
                =date_format(TO_DATE(b.DATE_OF_HRBP_HANDOVER),'yyyy-MM-dd') ''')

# COMMAND ----------

# DBTITLE 1,RMT Clearance same as HRBP for SL GTA,GTS,Canada Audit
if tableName=='hrbp_handover':
    spark.sql('''update kgsonedatadb.trusted_hist_gdc_onboarding_master set RMT_HANDOVER_TO_INDUSTRY=case when SERVICE_LINE in ('GTA','GTS','Canada Audit') then HRBP_HANDOVER_TO_RMT_TEAM else null end''')
...
if tableName=='ready_for_production':
    spark.sql('''MERGE INTO kgsonedatadb.trusted_hist_gdc_onboarding_master a 
               USING  kgsonedatadb.trusted_gdc_ready_for_production b
               ON a.PSID=b.PSID and a.SERVICE_LINE not in ('GTA','GTS','Canada Audit')
               WHEN MATCHED  THEN  
...
# DBTITLE 1,Candidate_Status
# IF(Emp Name="","",
# IF(AND(BGV Clear Date<>"",L&D handover to Risk Team="",Target Training Completion Date>=TODAY()),"In Training (Scheduled)",
# IF(AND(BGV Clear Date<>"",L&D handover to Risk Team="",Target Training Completion Date<TODAY()),"In Training (Delay)",
# IF(AND(BGV Clear Date="",L&D handover to Risk Team="",Target Training Completion Date>=TODAY()),"BGV Pending & In Training (Scheduled)",
# IF(AND(BGV Clear Date="",L&D handover to Risk Team="",Target Training Completion Date<TODAY()),"BGV Pending & In Training (Delay)",
# IF(AND(BGV Clear Date="",Risk Team handover to HRBP=""),"BGV Pending & Risk Team clearance pending",
# IF(AND(BGV Clear Date="",Risk Team handover to HRBP<>""),"BGV Pending & HRBP clearance pending",
# IF(AND(Target Risk Clearance Date>=TODAY(),Risk Team handover to HRBP=""),"Risk Team clearance pending (Scheduled)",
# IF(AND(Target Risk Clearance Date<TODAY(),Risk Team handover to HRBP=""),"Risk Team clearance pending (Delay)",
# IF(AND(HRBP handover to RMT="",(Risk Team handover to HRBP+1)>=TODAY()),"With HRBP (Scheduled)",
# IF(AND(HRBP handover to RMT="",(Risk Team handover to HRBP+1)<TODAY()),"With HRBP (Delay)",
# IF(AND(RMT handover to industry="",(HRBP handover to RMT+1)>=TODAY()),"With RMT (Scheduled)",
# IF(AND(RMT handover to industry="",(HRBP handover to RMT+1)<TODAY()),"With RMT (Delay)",
...
spark.sql('''Update kgsonedatadb.trusted_hist_gdc_onboarding_master 
            SET CANDIDATES_STATUS= CASE
                                        ...
                                        WHEN ((TARGET_RISK_CLEARANCE_DATE>=GETDATE()) and
                                        (RISK_TEAM_HANDOVER_TO_HRBP is Null or RISK_TEAM_HANDOVER_TO_HRBP ="")) then "Risk Team clearance pending (Scheduled)"

                                        WHEN ((TARGET_RISK_CLEARANCE_DATE<GETDATE()) and
                                        (RISK_TEAM_HANDOVER_TO_HRBP is Null or RISK_TEAM_HANDOVER_TO_HRBP ="")) then "Risk Team clearance pending (Delay)"

                                        WHEN ((HRBP_HANDOVER_TO_RMT_TEAM is Null or HRBP_HANDOVER_TO_RMT_TEAM ="")
                                        and  ((CAST(RISK_TEAM_HANDOVER_TO_HRBP AS DATE)+INTERVAL 1 DAY)>=GETDATE()))) then
                                        "With HRBP (Scheduled)"
                                        ...
                                        WHEN ((RMT_HANDOVER_TO_INDUSTRY is Null or RMT_HANDOVER_TO_INDUSTRY ="") and
                                        ((CAST(HRBP_HANDOVER_TO_RMT_TEAM AS DATE)+INTERVAL 1 DAY)>=GETDATE())) then "With RMT (Scheduled)"

                                        WHEN ((RMT_HANDOVER_TO_INDUSTRY is Null or RMT_HANDOVER_TO_INDUSTRY ="") and
                                        ((CAST(HRBP_HANDOVER_TO_RMT_TEAM AS DATE)+INTERVAL 1 DAY)<GETDATE())) then "With RMT (Delay)"
...
spark.sql('''
    UPDATE kgsonedatadb.trusted_hist_gdc_onboarding_master 
    SET CANDIDATES_STATUS =
        CASE 
            WHEN (EMP_NAME IS NULL OR EMP_NAME = '') THEN ''
            WHEN (BGV_CLEAR_DATE IS NOT NULL AND LND_HANDOVER_TO_RISK_TEAM IS NULL AND TARGET_TRAINING_COMPLETION_DATE >= CURRENT_DATE()) THEN 'In Training (Scheduled)'
            WHEN (BGV_CLEAR_DATE IS NOT NULL AND LND_HANDOVER_TO_RISK_TEAM IS NULL AND TARGET_TRAINING_COMPLETION_DATE < CURRENT_DATE()) THEN 'In Training (Delay)'
            WHEN (BGV_CLEAR_DATE IS NULL AND LND_HANDOVER_TO_RISK_TEAM IS NULL AND TARGET_TRAINING_COMPLETION_DATE >= CURRENT_DATE()) THEN 'BGV Pending & In Training (Scheduled)'
            WHEN (BGV_CLEAR_DATE IS NULL AND LND_HANDOVER_TO_RISK_TEAM IS NULL AND TARGET_TRAINING_COMPLETION_DATE < CURRENT_DATE()) THEN 'BGV Pending & In Training (Delay)'
            WHEN (BGV_CLEAR_DATE IS NULL AND RISK_TEAM_HANDOVER_TO_HRBP IS NULL) THEN 'BGV Pending & Risk Team clearance pending'
            WHEN (BGV_CLEAR_DATE IS NULL AND RISK_TEAM_HANDOVER_TO_HRBP IS NOT NULL) THEN 'BGV Pending & HRBP clearance pending'
            WHEN (TARGET_RISK_CLEARANCE_DATE >= CURRENT_DATE() AND RISK_TEAM_HANDOVER_TO_HRBP IS NULL) THEN 'Risk Team clearance pending (Scheduled)'
            WHEN (TARGET_RISK_CLEARANCE_DATE < CURRENT_DATE() AND RISK_TEAM_HANDOVER_TO_HRBP IS NULL) THEN 'Risk Team clearance pending (Delay)'
            WHEN (HRBP_HANDOVER_TO_RMT_TEAM IS NULL AND date_add(CAST(RISK_TEAM_HANDOVER_TO_HRBP AS DATE) , INTERVAL 1 DAY) >= CURRENT_DATE()) THEN 'With HRBP (Scheduled)'
            WHEN (HRBP_HANDOVER_TO_RMT_TEAM IS NULL AND date_add(CAST(RISK_TEAM_HANDOVER_TO_HRBP AS DATE) , INTERVAL 1 DAY) < CURRENT_DATE()) THEN 'With HRBP (Delay)'
            WHEN (RMT_HANDOVER_TO_INDUSTRY IS NULL AND date_add(CAST(HRBP_HANDOVER_TO_RMT_TEAM AS DATE) , INTERVAL 1 DAY) >= CURRENT_DATE()) THEN 'With RMT (Scheduled)'
            WHEN (RMT_HANDOVER_TO_INDUSTRY IS NULL AND date_add(CAST(HRBP_HANDOVER_TO_RMT_TEAM AS DATE) , INTERVAL 1 DAY) < CURRENT_DATE()) THEN 'With RMT (Delay)'
            ELSE 'Handed over to Business'
        END
''')
...
# if col("TIME_TAKEN_TO_RECEIVE_LND_CLEARANCE_DATE_FROM_LAPTOP_RECEIVED_DATE")  <= col( "Total_days")   then "0"
#earlier it was no delay instead of 0.
...
spark.sql('''MERGE INTO kgsonedatadb.trusted_hist_gdc_onboarding_master a
            ...
            WHEN MATCHED  THEN   
                ...
                WHEN ( a.TIME_TAKEN_TO_RECEIVE_LND_CLEARANCE_DATE_FROM_LAPTOP_RECEIVED_DATE <= b.Total_days) then "0"
