...
df_emp_dump = spark.sql("select * from kgsonedatadb.trusted_hist_headcount_employee_dump where File_Date='20221214'")

df_thanks_dump = spark.sql("select * from kgsonedatadb.trusted_hist_employee_engagement_thanks_dump where file_date='20221231' and upper(Reward_Status) in ('APPROVED','APPROVAL NOT REQUIRED')")
