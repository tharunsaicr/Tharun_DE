...
if (tableName == 'card_swipe_master_data'):
    currentDf = spark.sql("select * from kgsonedatadb.trusted_admin_transaction_report")
...
if (tableName == 'card_swipe_master_data'):
    currentDf = currentDf.filter(
        ...
        (
            ...
            upper(trim(currentDf.COMPANY)).rlike('.*CALIBER.*$') 
            )
        ) 
        &
        ...
        (
            ...
            upper(trim(currentDf.DEPARTMENT)).rlike('.*.*$') |
...
if tableName == 'card_swipe_master_data':
    ...
    employeeDf = spark.sql("select EMP_ID, NAME, EMAIL_ID, DEPARTMENT AS EMP_DEPARTMENT, BU2 as emp_BU from kgsonedatadb.trusted_admin_employee_data")
    employeeDf = employeeDf.dropDuplicates()

    currentDf = currentDf.join(employeeDf, ((upper(currentDf.PAYROLL_NUM)== upper(employeeDf.EMP_ID)) & (currentDf.BU2.isNull())) , "left")

    currentDf = currentDf.withColumn('BU2', when(((currentDf.BU2.isNull()) | (currentDf.BU2 == '-')), col('emp_BU')).otherwise(col('BU2')))
    currentDf = currentDf.withColumn('DEPARTMENT', when(((currentDf.DEPARTMENT.isNull()) | (currentDf.DEPARTMENT == '-')), col('EMP_DEPARTMENT')).otherwise(col('DEPARTMENT')))
    
    currentDf = currentDf.drop(*('EMP_ID', 'NAME', 'EMAIL_ID', 'EMP_DEPARTMENT', 'emp_BU'))
...
if tableName == 'card_swipe_master_data':
    ...
    cwkDf = spark.sql("select EMP_ID, NAME, EMAIL_ID, DEPARTMENT AS EMP_DEPARTMENT, BU2 as emp_BU from kgsonedatadb.trusted_admin_cwk_data")
    cwkDf = cwkDf.dropDuplicates()

    currentDf = currentDf.join(cwkDf, ((upper(currentDf.PAYROLL_NUM)== upper(cwkDf.EMP_ID)) & (currentDf.BU2.isNull())) , "left")
...
if tableName == 'card_swipe_master_data':
    ...
    loanedDf = spark.sql("select EMP_ID, NAME, EMAIL_ID, DEPARTMENT AS EMP_DEPARTMENT, BU2 as emp_BU from kgsonedatadb.trusted_admin_loaned_data")
    loanedDf = loanedDf.dropDuplicates()

    currentDf = currentDf.join(loanedDf, ((upper(currentDf.PAYROLL_NUM)== upper(loanedDf.EMP_ID)) & (currentDf.BU2.isNull())) , "left")
...
if tableName == 'card_swipe_master_data':
    ...
    currentDf = currentDf.join(monthlyEmployeeDf, ((upper(currentDf.PAYROLL_NUM)== upper(monthlyEmployeeDf.Employee_Number)) & (currentDf.BU2.isNull())) , "left")
    currentDf = currentDf.withColumn('DEPARTMENT', when(((currentDf.DEPARTMENT.isNull()) | (currentDf.DEPARTMENT == '-')), col('Cost_centre')).otherwise(col('DEPARTMENT')))
    currentDf = currentDf.drop(*('Employee_Number', 'Full_Name', 'Cost_centre', 'Source'))
    ...
    currentDf = currentDf.join(monthlyResignedDf, ((upper(currentDf.PAYROLL_NUM)== upper(monthlyResignedDf.Employee_Number)) & (currentDf.BU2.isNull())) , "left")
    currentDf = currentDf.withColumn('DEPARTMENT', when(((currentDf.DEPARTMENT.isNull()) | (currentDf.DEPARTMENT == '-')), col('Cost_centre')).otherwise(col('DEPARTMENT')))
    currentDf = currentDf.drop(*('Employee_Number', 'Full_Name', 'Cost_centre', 'Source'))
    ...
    currentDf = currentDf.join(monthlyContingentWorker, ((upper(currentDf.PAYROLL_NUM)== upper(monthlyContingentWorker.Employee_Number)) & (currentDf.BU2.isNull())) , "left")
    currentDf = currentDf.withColumn('DEPARTMENT', when(((currentDf.DEPARTMENT.isNull()) | (currentDf.DEPARTMENT == '-')), col('Cost_centre')).otherwise(col('DEPARTMENT')))
    currentDf = currentDf.drop(*('Employee_Number', 'Full_Name', 'Cost_centre', 'Source'))
    ...
    currentDf = currentDf.join(monthlyAcademicTrainee, ((upper(currentDf.PAYROLL_NUM)== upper(monthlyAcademicTrainee.Employee_Number)) & (currentDf.BU2.isNull())) , "left")
    currentDf = currentDf.withColumn('DEPARTMENT', when(((currentDf.DEPARTMENT.isNull()) | (currentDf.DEPARTMENT == '-')), col('Cost_centre')).otherwise(col('DEPARTMENT')))
    currentDf = currentDf.drop(*('Employee_Number', 'Full_Name', 'Cost_centre', 'Source'))
    ...
    currentDf = currentDf.join(monthlyLoanedDF, ((upper(currentDf.PAYROLL_NUM)== upper(monthlyLoanedDF.Employee_Number)) & (currentDf.BU2.isNull())) , "left")
    currentDf = currentDf.withColumn('DEPARTMENT', when(((currentDf.DEPARTMENT.isNull()) | (currentDf.DEPARTMENT == '-')), col('Cost_centre')).otherwise(col('DEPARTMENT')))
    currentDf = currentDf.drop(*('Employee_Number', 'Full_Name', 'Cost_centre', 'Source'))
    ...
    currentDf = currentDf.join(monthlyContingentWorkerResignedDf, ((upper(currentDf.PAYROLL_NUM)== upper(monthlyContingentWorkerResignedDf.Employee_Number)) & (currentDf.BU2.isNull())) , "left")
    currentDf = currentDf.withColumn('DEPARTMENT', when(((currentDf.DEPARTMENT.isNull()) | (currentDf.DEPARTMENT == '-')), col('Cost_centre')).otherwise(col('DEPARTMENT')))
    currentDf = currentDf.drop(*('Employee_Number', 'Full_Name', 'Cost_centre', 'Source'))

    #LOANED RESIGNED
    monthlyLoanedResignedDf = spark.sql("select Employee_Number,Employee_Name as Full_Name,Cost_Center as Cost_centre,'Loned_Staff_Resigned' as Source from kgsonedatadb.trusted_headcount_monthly_loaned_staff_resigned")
    monthlyLoanedResignedDf.dropDuplicates()
    currentDf = currentDf.join(monthlyLoanedResignedDf, ((upper(currentDf.PAYROLL_NUM)== upper(monthlyLoanedResignedDf.Employee_Number)) & (currentDf.BU2.isNull())) , "left")
    currentDf = currentDf.withColumn('DEPARTMENT', when(((currentDf.DEPARTMENT.isNull()) | (currentDf.DEPARTMENT == '-')), col('Cost_centre')).otherwise(col('DEPARTMENT')))
    currentDf = currentDf.drop(*('Employee_Number', 'Full_Name', 'Cost_centre', 'Source'))
    ...
    currentDf = currentDf.join(monthlySecondeeOutwardDf, ((upper(currentDf.PAYROLL_NUM)== upper(monthlySecondeeOutwardDf.Employee_Number)) & (currentDf.BU2.isNull())) , "left")
    currentDf = currentDf.withColumn('DEPARTMENT', when(((currentDf.DEPARTMENT.isNull()) | (currentDf.DEPARTMENT == '-')), col('Cost_centre')).otherwise(col('DEPARTMENT')))
    currentDf = currentDf.drop(*('Employee_Number', 'Full_Name', 'Cost_centre', 'Source'))
...
if (tableName == 'card_swipe_master_data'):
    currentDf = currentDf.select('LOCATION','DEPARTMENT','PAYROLL_NUM','NAME___REG_NUM','JOB_TITLE','TRANSACTION_TIME','BU2','Office_Location','FILE_DATE')
