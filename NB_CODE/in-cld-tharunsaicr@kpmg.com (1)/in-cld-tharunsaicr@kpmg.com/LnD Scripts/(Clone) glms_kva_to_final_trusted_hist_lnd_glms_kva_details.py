...
if (sourceFile =="glms_details"):
    if (spark._jsparkSession.catalog().tableExists('kgsonedatadb', 'test_trusted_hist_lnd_glms_details')):
        df_trusted_curr_lnd_glms = spark.sql("select Item_ID,Training_Category,Item_Title,KBS_Classification_,Item_Type,Assignment_Type,Security_Domain__Item_,CPD_CPE_Hours__Item_,Item_Revision_Date,Class_ID,Scheduled_Offering_Start_Date,SO_Item_Start_Time,Class_End_Date,Scheduled_Offering_End_Time,Completion_Date,Period,Completion_Time,Completion_Status_ID,Completion_Status_Description,Total_Hours,Credit_Hours,Contact_Hours,CPD_CPE_Hours_Awarded,Instructor as Primary_Instructor,Last_Update_User_ID,Last_Update_Timestamp,Last_Update_Time,Comments,Local_HR_ID,First_Name,Last_Name,Middle_Name,Email_Address__User_,User_ID,Account_ID,User_Status,Job_Code_ID,Job_Code_Description,Job_Title,Position,Job_Name,Level_Wise,Country_Region,Office_Location_ID,Department,GlobalFunction as Global_Function,Organisation_Description,CC,BU,Subfunction,Employee_Class_Description,Local_Job_Level,Is_Client_Facing,Manager_Email_Address,Last_Hire_Date,Local_Function,Local_Service_Line,Cost_Centre as Cost_Center,ORG_ID,Geo,Service_Line as SL,Location,Item_Type_Category,' ' as Full_Name,Quarter,Mapping_Original as Original_Levelwise_Mapping,File_Type,File_Date from (select rank() over(partition by file_date order by file_date desc) as rank, * from kgsonedatadb.test_trusted_hist_lnd_glms_details where file_date = (select max(file_date) from kgsonedatadb.test_trusted_hist_lnd_glms_details where to_date(File_Date,'yyyyMMdd') <= '"+str(convertedFileDate)+"'"+")) hist where rank = 1")

        df_glms_kva=df_trusted_curr_lnd_glms.withColumn("Last_Update_Timestamp",df_trusted_curr_lnd_glms.Last_Update_Timestamp.cast(DateType()))
...
dbutils.notebook.run("/Users/in-cld-tharunsaicr@kpmg.com/LnD Scripts/(Clone) Data_Cleanup",6000, {'DeltaTableName':tableName, 'ProcessName':processName, 'LayerName':layerName})