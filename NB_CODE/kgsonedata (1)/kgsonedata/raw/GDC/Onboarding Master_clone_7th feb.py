...
if tableName=='analytics':
    ...
    temp_df=spark.sql("SELECT EMP_NAME,CAST(NULL AS int) AS EMP_ID,PSID,DESIGNATION,LOCATION,DOJ,GENDER,CAST(NULL AS STRING) AS UG,CAST(NULL AS STRING) AS PG,CAST(NULL AS STRING) AS HIGHEST_QUALIFICATION,INDUSTRY_GROUP_COST_CENTER,CAST(NULL AS STRING) AS SERVICE_LINE,INDUSTRY,CASE WHEN EMP_CATEGORY=='Staff on Probation' THEN 'FTE' WHEN EMP_CATEGORY=='Fixed Term Staff' THEN 'FTS' END AS EMP_CATEGORY,LATERAL_CAMPUS,CAST(NULL AS STRING) AS ATTENDED_GENESIS_KBS,CAST(NULL AS INT) AS NO_OF_COURSES_PENDING,CAST(NULL AS STRING) AS BGV_STATUS,CAST(NULL AS DATE) AS BGV_CLEAR_DATE,CAST(NULL AS DATE) AS LND_HANDOVER_TO_RISK_TEAM,CAST(NULL AS DATE) AS RISK_TEAM_HANDOVER_TO_HRBP,CAST(NULL AS DATE) AS HRBP_HANDOVER_TO_RMT_TEAM,CAST(NULL AS DATE) AS RMT_HANDOVER_TO_INDUSTRY,'BCP_access_blocked' AS BCP,INDIA_ID,VDI_ID,CAST(NULL AS DATE)  AS VDI_ID_RECEIVED_DATE,CAST(NULL AS STRING) AS JOINERS_STATUS,CAST(NULL AS STRING) AS BYOD,CAST(NULL AS STRING) AS LAPTOP_DELIVERY_STATUS,CAST(NULL AS DATE) AS LAPTOP_DISPATCH_DATE,CAST(NULL AS DATE) AS LAPTOP_REACHED_ON,CAST(NULL AS STRING) AS CANDIDATES_STATUS,DATE_FORMAT(DOJ,'MMM-yyyy') AS DOJ_BY_MONTH,CAST(NULL AS STRING) AS ONBOARD_SUMMARY_DOJ,CAST(NULL AS DATE) AS TARGET_TRAINING_COMPLETION_DATE,CAST(NULL AS DATE) AS TARGET_RISK_CLEARANCE_DATE,CAST(NULL AS DATE) AS BGV_SLA_DATE,CANDIDATE_PERSONAL_EMAIL_ID,'Office' as CANDIDATE_JOINED_OFFICE_HOME,CAST(NULL AS STRING) AS PRIOR_INDUSTRY,PM,PM_EMAIL_ID,BUDDY_NAME,BUDDY_EMAIL_ID,CAST(NULL AS INT) AS TIME_TAKEN_TO_RECEIVE_LND_CLEARANCE_DATE_FROM_LAPTOP_RECEIVED_DATE,CAST(NULL AS INT) AS LND_DELAY_IN_DAYS,CAST(NULL AS STRING) AS DELAY_REASONS,CAST(NULL AS INT) AS DAYS_TAKEN_FROM_DOJ_TILL_HANDOVER_TO_BUSINESS,VDI_ID_RECEIVED_DATE AS File_Date,CAST( NULL AS STRING) AS LnD_Team_Comments ,CAST (NULL AS STRING) AS Risk_Team_Comments,COALESCE(EMP_NAME,PSID,DESIGNATION,LOCATION,DOJ,GENDER,INDUSTRY_GROUP_COST_CENTER,INDUSTRY,EMP_CATEGORY,LATERAL_CAMPUS,INDIA_ID,VDI_ID,VDI_ID_RECEIVED_DATE,CANDIDATE_PERSONAL_EMAIL_ID,PM,PM_EMAIL_ID,BUDDY_NAME,BUDDY_EMAIL_ID) IS NOT NULL THEN 'Complete' ELSE 'Incomplete' AS DATABASE_STATUS FROM temp_df")
...
# DBTITLE 1,Update Candidate_Status
# Candidate status=
# IF(Emp Name="","",IF(AND(BGV Clear Date<>"",L&D handover to Risk Team="",Target Training Completion Date>=TODAY()),"In Training (Scheduled)",
...
spark.sql('''Update kgsonedatadb.trusted_hist_gdc_onboarding_master 
            SET CANDIDATES_STATUS= CASE 
                                        ...
                                        and  ((RISK_TEAM_HANDOVER_TO_HRBP+INTERVAL 1 DAY)>=GETDATE())) then
                                        "With HRBP (Scheduled)"
                                        ...
                                        ((HRBP_HANDOVER_TO_RMT_TEAM+INTERVAL 1 DAY)>=GETDATE())) then "With RMT (Scheduled)"

                                        WHEN ((RMT_HANDOVER_TO_INDUSTRY is Null or RMT_HANDOVER_TO_INDUSTRY ="") and
                                        ((HRBP_HANDOVER_TO_RMT_TEAM+INTERVAL 1 DAY)<GETDATE())) then "With RMT (Delay)"
...
spark.sql('''Update kgsonedatadb.trusted_hist_gdc_onboarding_master
            ...
            SET TIME_TAKEN_TO_RECEIVE_LND_CLEARANCE_DATE_FROM_LAPTOP_RECEIVED_DATE= CASE 
                                        WHEN ((LND_HANDOVER_TO_RISK_TEAM is Null) and (TARGET_TRAINING_COMPLETION_DATE <= current_date()) and (LOCATION!='Hyderabad')) then (current_deate() - DOJ)
