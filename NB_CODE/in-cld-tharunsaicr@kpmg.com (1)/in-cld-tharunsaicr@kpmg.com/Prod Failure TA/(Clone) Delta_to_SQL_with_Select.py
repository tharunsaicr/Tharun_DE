...
if processName == "TA_post_hire":
    ...
    if table == "trusted_TA_post_hire_CWK":
        sampleDF=spark.sql('select Req__Identifier,Candidate_Identifier,Candidate_Name,Designation,Cost_Centre,BU,Geo,Recruiter_Name,Email,Hiring_Source,Project_Code,Start_Date,Joining_Location,Sub_Location,Location_Category,Exception_Approval_To_On_Board,ZONE__For_Delhi__NCR_,Phone_Number,Current_Address_With_landmark_where_Laptop_Can_be_Delivered,Candidate_Category__ROD_or_RPO_,Remarks,KGS_Email_ID,Status,Updated_DOJ,BGV_Reference_No_,BGV_Status,Report_Colour,Mandatory_Checks_Completed,All_Checks_Completed,Case_Remarks,Onboarding_Approval,Approval_Date,Joining_Status as Final_Joining_Status,Dated_On,File_Date,left(File_Date,8) as Month_Key from kgsonedatadb.'+table)
...
display(sampleDF.filter(sampleDF.Month_Key.isNull())) 
display(joinDf.filter(sampleDF.Month_Key.isNull())) 
display(cutoffDf)
...
# MAGIC %sql
# MAGIC INSERT INTO kgsonedatadb.config_hist_hrss_payroll_cutoff_date 
# MAGIC values ('24-Jan','2024-01-18','20230920','2023-09-20T19:09:41.000+0000')

# COMMAND ----------

# MAGIC %sql
# MAGIC select * from kgsonedatadb.config_hist_hrss_payroll_cutoff_date 
...
if processName == "admin":
    ...
    if table == "trusted_admin_card_swipe_master_data":
        sampleDF=spark.sql('''select `LOCATION`,DEPARTMENT AS BU,PAYROLL_NUMBER AS EMPLOYEE_ID,NAME___REG_NUMBER AS EMPLOYEE_NAME,JOB_TITLE AS LEVEL,TRANSACTION_TIME AS DATE,BU2,Dated_On,FILE_DATE,left(File_Date,8) as Month_Key from kgsonedatadb.'''+table)
    ...
    if table == "trusted_admin_booked_and_no_shows":
        sampleDF=spark.sql('''select `LOCATION`,BU,EMPLOYEE_ID,EMPLOYEE_NAME,STATUS2,FROM_DATE,STATUS,Dated_On,FILE_DATE,left(File_Date,8) as Month_Key from kgsonedatadb.'''+table)
