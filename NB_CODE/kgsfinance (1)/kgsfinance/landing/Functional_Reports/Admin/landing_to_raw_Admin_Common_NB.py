...
#commom nb for ingesting admin function cost,fy classification,fx rate and transport split regular sheets
...
if tableName=='function_cost':
    #read the csv file
    readDf =spark.read.format("csv").option("header","true").option("delimiter",",") \
...
current_df=current_df.withColumn("Dated_On", to_timestamp(lit(currentdatetime)))
...
row = current_df.count()
print("Row ",row)

# extracting number of columns from the Dataframe
column = len(current_df.columns)
print("Column ",column)
